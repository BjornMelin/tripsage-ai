...
Examples:
    ...
    parser.add_argument(
        "--security", 
        action="store_true", 
        help="Run security and RLS tests only"
    )
    
    parser.add_argument(
        "--migration", 
        action="store_true", 
        help="Run migration safety tests only"
    )
    ...
    try:
        ...
        if result["success"]:
            ...
            if result.get("analysis", {}).get("summary", {}).get("total_failed", 0) > 0:
                print(f"⚠️  {result['analysis']['summary']['total_failed']} tests failed")
                return 1
            return 0
        else:
...
def run_pytest_directly():
    ...
    if len(sys.argv) > 1:
        ...
        if test_type == "--pytest-performance":
            ...
            print("Unknown pytest option. Use --pytest-performance, --pytest-integration, or --pytest-all")
            return 1
    else:
