...
- **Path aliases:** Use semantic aliases for architectural boundaries:
  ...
  - **Relative imports:** Prefer relative imports (`./client-types`, `../utils`) within local feature slices (e.g., inside `src/domain/amadeus/*` or `src/ai/tools/server/*`). Use aliases when crossing architectural boundaries.
