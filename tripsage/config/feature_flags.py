...
class FeatureFlags(BaseSettings):
    ...
    redis_integration: IntegrationMode = Field(
        default=IntegrationMode.MCP, description="Redis integration mode"
    )
    supabase_integration: IntegrationMode = Field(
        default=IntegrationMode.MCP, description="Supabase integration mode"
    )

    # Web crawling services
    crawl4ai_integration: IntegrationMode = Field(
        default=IntegrationMode.MCP, description="Crawl4AI integration mode"
    )
    playwright_integration: IntegrationMode = Field(
        default=IntegrationMode.MCP, description="Playwright integration mode"
    )
