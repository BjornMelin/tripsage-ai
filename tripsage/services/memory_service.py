...
Based on comprehensive research from docs/REFACTOR/MEMORY_SEARCH/RESEARCH_DB_MEMORY_SEARCH.md
...
class TripSageMemoryService(ServiceProtocol):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        logger.info("TripSageMemoryService initialized", config_provider=self.config.get("vector_store", {}).get("provider"))
        ...
        return {
            "vector_store": {
                ...
                "config": {
                    ...
                    "database": settings.supabase_db_name or "postgres",
                    "table_name": "memories",
                    "pool_size": 20,
                    "max_overflow": 10,
                    "pool_timeout": 30,
    ...
    async def connect(self) -> None:
        ...
        try:
            ...
            logger.error("Failed to connect memory service", error=str(e))
            raise
    ...
    async def close(self) -> None:
        ...
        try:
            ...
            logger.error("Error closing memory service", error=str(e))
    ...
    async def health_check(self) -> bool:
        ...
        try:
            ...
            test_result = await self.memory.search(
    ...
    @with_error_handling(logger=logger, raise_on_error=False)
    async def add_conversation_memory(
        ...
        try:
            ...
            logger.error("Memory extraction failed", error=str(e), user_id=user_id)
    ...
    async def search_memories(
        ...
        try:
            ...
            logger.error("Memory search failed", error=str(e), user_id=user_id, query=query)
            return []
    ...
    async def update_user_preferences(
        ...
        try:
            ...
            preference_messages = [
                ...
                {
                    "role": "user",
                    "content": f"My travel preferences: {json.dumps(preferences, indent=2)}"
                }
            ]
            ...
            logger.info("User preferences updated", user_id=user_id, preferences_count=len(preferences))
            return result
            ...
            if memory_ids:
                ...
                for memory_id in memory_ids:
                    try:
                        ...
                        logger.warning(f"Failed to delete memory {memory_id}", error=str(e))
            else:
                ...
                for memory in all_memories.get("results", []):
                    try:
                        ...
                        logger.warning(f"Failed to delete memory {memory.get('id')}", error=str(e))
            
            # Clear user's cache
            self._invalidate_user_cache(user_id)
            
            logger.info("User memories deleted", user_id=user_id, deleted_count=deleted_count)
            return {"deleted_count": deleted_count, "success": True}
            
        except Exception as e:
            logger.error("Failed to delete user memories", error=str(e), user_id=user_id)
    ...
    def _analyze_destinations(self, context: Dict[str, List]) -> Dict[str, Any]:
        ...
        for memory in context.get("past_trips", []) + context.get("saved_destinations", []):
            ...
            for word in content.split():
                if word.capitalize() in ["Japan", "France", "Italy", "Spain", "Thailand", "USA", "UK"]:
    ...
    def _analyze_activities(self, context: Dict[str, List]) -> Dict[str, Any]:
        ...
        for memory in context.get("activity_preferences", []) + context.get("preferences", []):
            ...
            activity_keywords = ["museum", "beach", "hiking", "shopping", "dining", "nightlife", "culture"]
