...
class DuffelHTTPClient:
    ...
    def __init__(
        ...
        if api_key:
            ...
            if feature_flags.flights_integration.value == "direct" and settings.duffel_api_key:
        ...
        self._max_requests_per_minute = 100  # Conservative rate limit
    ...
    async def _make_request(
        ...
        while retry_count <= self.max_retries:
            try:
                logger.debug(f"Making {method} request to {url} (attempt {retry_count + 1})")
                ...
                try:
                    ...
                    raise DuffelAPIError(f"Failed to parse response JSON: {str(e)}")
    ...
    @with_error_handling
    async def search_flights(self, search_params: FlightSearchParams) -> FlightSearchResponse:
        ...
        try:
            ...
            for offer in offers_data:
                converted_offer = {
                    ...
                    "base_amount": float(offer.get("base_amount", 0)) if offer.get("base_amount") else None,
                    "tax_amount": float(offer.get("tax_amount", 0)) if offer.get("tax_amount") else None,
            ...
            search_response = FlightSearchResponse(
                ...
                currency=converted_offers[0]["total_currency"] if converted_offers else "USD",
                search_id=offer_request_data.get("id"),
                cheapest_price=min(offer["total_amount"] for offer in converted_offers) if converted_offers else None,
            )
            ...
            raise DuffelAPIError(f"Invalid response format: {str(e)}")
        except Exception as e:
    ...
    async def get_aircraft(self, aircraft_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            response_data = await self._make_request(
                method="GET",
                endpoint=f"/air/aircraft/{aircraft_id}"
            )
    ...
    @with_error_handling
    async def get_airports(self, search_params: AirportSearchParams) -> AirportSearchResponse:
