...
class FileProcessor:
    ...
    async def process_file(self, file: UploadFile, user_id: str) -> ProcessedFile:
        ...
        try:
            ...
            if not validation_result.is_valid:
                raise HTTPException(
                    status_code=400, detail=validation_result.error_message
                )
            ...
            metadata = await self._extract_file_metadata(
                storage_path, validation_result.detected_type
            )
    ...
    async def process_batch(
        ...
        if errors and not processed_files:
            ...
            raise HTTPException(
                status_code=400,
                detail=f"All files failed to process: {'; '.join(errors)}",
            )
        elif errors:
    ...
    async def get_storage_stats(self, user_id: str) -> Dict[str, Any]:
        ...
        if not user_dir.exists():
            return {"total_files": 0, "total_size": 0, "storage_used": "0 MB"}
