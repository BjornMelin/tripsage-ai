"""WebSocket message broadcasting service with DragonflyDB integration.
...
class WebSocketBroadcaster:
    ...
    async def broadcast_to_connection(
        self, connection_id: str, event: WebSocketEvent, priority: int = 1
    ) -> bool:
        ...
        message = BroadcastMessage(
            id=f"conn_{connection_id}_{event.id}",
            event=event,
    ...
    async def broadcast_to_user(
        self, user_id: UUID, event: WebSocketEvent, priority: int = 1
    ) -> bool:
    ...
    async def broadcast_to_session(
        self, session_id: UUID, event: WebSocketEvent, priority: int = 1
    ) -> bool:
        ...
        """
        message = BroadcastMessage(
            id=f"session_{session_id}_{event.id}",
            event=event,
            target_type="session",
    ...
    async def broadcast_to_channel(
        self, channel: str, event: WebSocketEvent, priority: int = 1
    ) -> bool:
        ...
        """
        message = BroadcastMessage(
            id=f"channel_{channel}_{event.id}",
            event=event,
            target_type="channel",
            target_id=channel,
            created_at=datetime.utcnow(),
    ...
    async def broadcast_to_all(self, event: WebSocketEvent, priority: int = 2) -> bool:
