...
Supports both MCP and direct HTTP integration via feature flags as outlined in Issue #163.
"""
...
class FlightService:
    ...
    When flights_integration is set to DIRECT, uses DuffelHTTPClient for direct API calls.
    ...
    async def _convert_api_models_to_flight_search_params(
        ...
        return FlightSearchParams(
            ...
            departure_date=request.departure_date.isoformat() if hasattr(request.departure_date, 'isoformat') else str(request.departure_date),
            return_date=request.return_date.isoformat() if request.return_date and hasattr(request.return_date, 'isoformat') else str(request.return_date) if request.return_date else None,
    ...
    async def _convert_duffel_response_to_api_models(
        ...
        for offer in duffel_response.offers:
            ...
            if offer.get("slices"):
                for slice_data in offer["slices"]:
                    for segment in slice_data.get("segments", []):
                        segments.append({
                            "departure_airport": segment.get("origin", {}).get("iata_code", ""),
                            "arrival_airport": segment.get("destination", {}).get("iata_code", ""),
            ...
            flight_offer = FlightOffer(
                ...
                airline=segments[0].get("flight_number", "").split()[0] if segments else "Unknown",
                airline_name="Unknown Airline",  # Would need airline lookup
                price=offer["total_amount"],
                currency=offer["total_currency"],
                cabin_class=request.cabin_class,
                stops=max(0, len(segments) - 1) if segments else 0,
                duration_minutes=sum(seg.get("duration_minutes", 180) for seg in segments),
                segments=segments,
        ...
        return FlightSearchResponse(
            ...
            min_price=min(offer.price for offer in flight_offers) if flight_offers else None,
            max_price=max(offer.price for offer in flight_offers) if flight_offers else None,
    ...
    async def search_flights(
        ...
        if feature_flags.flights_integration == IntegrationMode.DIRECT:
            ...
            try:
                ...
                search_params = await self._convert_api_models_to_flight_search_params(request)
                
                # Make direct API call
                duffel_response = await self.duffel_client.search_flights(search_params)
                
                # Convert back to API format
                return await self._convert_duffel_response_to_api_models(duffel_response, request)
            ...
            try:
                ...
                return await self._convert_duffel_response_to_api_models(result, request)
    ...
    async def _get_mock_flight_response(self, request: FlightSearchRequest) -> FlightSearchResponse:
