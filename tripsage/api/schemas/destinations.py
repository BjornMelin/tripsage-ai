...
class DestinationSearchRequest(BaseModel):
    ...
    categories: Optional[List[DestinationCategory]] = Field(default=None, description="Categories to filter by")
    min_safety_rating: Optional[float] = Field(default=None, ge=0, le=5, description="Minimum safety rating (0-5)")
    travel_month: Optional[str] = Field(default=None, description="Month of travel (for weather filtering)")
    limit: int = Field(10, ge=1, le=50, description="Maximum number of results to return")
    include_weather: bool = Field(default=False, description="Whether to include weather information")
    include_attractions: bool = Field(default=False, description="Whether to include points of interest")
...
class DestinationDetailsRequest(BaseModel):
    ...
    include_weather: bool = Field(default=True, description="Whether to include weather information")
    include_attractions: bool = Field(default=True, description="Whether to include points of interest")
    travel_date: Optional[date] = Field(default=None, description="Travel date for seasonal information")
...
class SavedDestinationRequest(BaseModel):
    ...
    notes: Optional[str] = Field(default=None, description="Notes about this destination")
    visit_schedule: Optional[DestinationVisitSchedule] = Field(default=None, description="Visit schedule details")
...
class DestinationSuggestionRequest(BaseModel):
    ...
    interests: List[str] = Field(description="User interests (e.g., 'beaches', 'hiking', 'history')")
    travel_dates: Optional[List[date]] = Field(default=None, description="Potential travel dates")
    budget_range: Optional[Dict[str, float]] = Field(
        default=None,
        description="Budget range in USD (e.g., {'min': 500, 'max': 2000})",
    )
    trip_duration_days: Optional[int] = Field(default=None, ge=1, description="Trip duration in days")
    preferred_continents: Optional[List[str]] = Field(default=None, description="Preferred continents")
    preferred_climate: Optional[str] = Field(default=None, description="Preferred climate")
