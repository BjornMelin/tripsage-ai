...
# File configuration - Following KISS principle: start simple
MAX_FILE_SIZE = 10 * 1024 * 1024  # 10MB per file
MAX_FILES_PER_SESSION = 50  # 50MB total per session
ALLOWED_EXTENSIONS = {
...
# Module-level dependency to avoid B008 warnings
...
# Module-level dependency for file processor
...
class FileUploadResponse(BaseModel):
    ...
    extracted_data: Optional[dict] = Field(None, description="Extracted travel data")
...
async def upload_file(
    ...
    try:
        ...
        logger.info(
            ...
            f"({file.size} bytes) for user {current_user.id}"
        )
...
async def upload_files_batch(
    ...
    for file in files:
        try:
            ...
            # Process file
            result = await processor.process_file(file, current_user.id)
            processed_files.append(FileUploadResponse(
                file_id=result.file_id,
                filename=result.filename,
                size=result.size,
                content_type=result.content_type,
...
async def get_file_metadata(
    ...
    """
    try:
        file_info = await processor.get_file_info(file_id, current_user.id)
        if not file_info:
            raise HTTPException(
