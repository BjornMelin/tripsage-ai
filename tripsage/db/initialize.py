...
async def initialize_databases(
    ...
    try:
        ...
        if run_migrations_on_startup:
            ...
            logger.info(f"Neo4j migrations: {neo4j_succeeded} succeeded, {neo4j_failed} failed")
    ...
    try:
        ...
        if result.result and "rows" in result.result:
            ...
            results["sql"]["missing_tables"] = [
                t for t in ["users", "trips", "migrations"] 
        ...
        if result.result and "entities" in result.result:
            schema_entities = [
                e["name"] for e in result.result["entities"]
