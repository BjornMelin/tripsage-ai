...
def with_error_handling(func: F) -> F:
    ...
    This decorator is particularly useful for agent tools, as it ensures
    that errors are properly caught, logged, and a standardized error
    ...
    async def wrapper(*args: Any, **kwargs: Any) -> Any:
        try:
            ...
            log_exception(e)
            # Format the error as a user-friendly response
            if isinstance(e, MCPError):
                error_message = f"Error from {e.server}: {e.message}"
            elif isinstance(e, APIError):
                error_message = f"Error from {e.service}: {e.message}"
            else:
