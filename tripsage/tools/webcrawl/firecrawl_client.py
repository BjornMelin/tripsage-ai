...
class FirecrawlClient:
    ...
    def __init__(self):
        ...
        self.api_key = settings.firecrawl_mcp.api_key
        self.base_url = settings.firecrawl_mcp.endpoint
        self.timeout = settings.firecrawl_mcp.timeout

        if not self.api_key:
            logger.warning("FIRECRAWL_API_KEY not set in environment variables")

        if not self.base_url:
            # Use default MCP base URL
            self.base_url = "https://api.firecrawl.dev/v1"
            logger.info(f"Using default Firecrawl API base URL: {self.base_url}")
    ...
    async def scrape_url(
        ...
        try:
            ...
            payload = {
                ...
                "includeTags": (
                    specific_selector.split(",") if specific_selector else None
                ),
            ...
            error_msg = (
                f"Firecrawl HTTP error {e.response.status_code}: {e.response.text}"
            )
            logger.error(error_msg)
            return {"success": False, "error": error_msg}
    ...
    async def search_web(
        self, query: str, depth: str = "standard", limit: int = 5
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            payload = {"query": query, "limit": limit, "scrapeOptions": scrape_options}
        ...
        try:
            ...
            payload = {
                ...
                "extract": {
                    ...
                    "prompt": (
                        "Extract the current price, currency, product name, and "
                        "availability status from this product page."
                    ),
                },
            }
    ...
    async def crawl_blog(
        ...
        try:
            ...
            if max_pages > 1:
                ...
                async with httpx.AsyncClient() as client:
                    response = await client.post(
                        ...
                        json=payload,
                        headers=headers,
                        timeout=120.0,  # Longer timeout for crawl
                    )
                    response.raise_for_status()
                    result = response.json()
            ...
            # For single page, use the extract endpoint with
            # schema based on extract_type
            else:
                endpoint = f"{self.base_url}/scrape"
                ...
                if extract_type == "insights":
                    extract_prompt = (
                        "Extract key travel insights, advice, and highlights "
                        "from this travel blog."
                    )
                elif extract_type == "itinerary":
                    extract_prompt = (
                        "Extract the itinerary, day-by-day activities, and travel "
                        "route from this travel blog."
                    )
                elif extract_type == "tips":
                    extract_prompt = (
                        "Extract practical tips, warnings, and recommendations "
                        "from this travel blog."
                    )
                elif extract_type == "places":
                    extract_prompt = (
                        "Extract all mentioned places, attractions, restaurants, "
                        "and hotels from this travel blog."
                    )
    ...
    def _format_price_result(self, result: Dict[str, Any], url: str) -> Dict[str, Any]:
        ...
        if not result.get("success", False):
            return {
                ...
                "url": url,
                "error": result.get(
                    "error", "Unknown error in Firecrawl price monitoring"
                ),
            }
    ...
    def _format_events_result(
        self, result: Dict[str, Any], destination: str
    ) -> Dict[str, Any]:
        ...
        if not result.get("success", False):
            return {
                ...
                "error": result.get(
                    "error", "Unknown error in Firecrawl events search"
                ),
            }

        # In a real implementation, we would parse the results to extract structured
        # event data. For this example, we'll just return the research results
