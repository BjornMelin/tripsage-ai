...
- Zod v4 schemas are centralized under `frontend/src/domain/schemas/*` and re-exported via `frontend/src/domain/types/index.ts`; flights currently lack a dedicated domain schema and rely on agent-specific schemas in `@schemas/agents` plus Supabase row schemas in `domain/schemas/supabase.ts`.  
- AI flight tooling lives in `frontend/src/ai/tools/server/flights.ts` (Duffel offer search) and generic planning schemas (`frontend/src/ai/tools/schemas/planning.ts`), but there is no shared flight domain model or provider mapper.  
- ADR-0012 defined Python/FastAPI DTOs; the stack is now frontend-only with Next.js 16 + AI SDK v6. We need a single TS/Zod source of truth that fits existing domain/schema conventions and tool schema locations (`frontend/src/ai/tools/schemas/*`).

## Decision

1) **Create flight domain module aligned with existing pattern**  
   - Add `frontend/src/domain/schemas/flights.ts` with sections:  
   ...
   - Re-export types from `frontend/src/domain/types/index.ts` (e.g., `FlightOffer`, `FlightSearchInput`, `FlightSearchResult`).  
...
2) **Add flight domain logic folder**  
   - Create `frontend/src/domain/flights/` with:  
...
3) **Tool schema alignment**  
   - Add `frontend/src/ai/tools/schemas/flights.ts` that reuses `FlightSearchInput`/`FlightSearchResult` from `@schemas/flights` (no parallel literals).  
   - Update `frontend/src/ai/tools/server/flights.ts` to import these schemas and use `domain/flights/mappers` for normalization instead of returning raw Duffel payloads. Keep guardrails (`cache`, `rateLimit`, `telemetry`) but move error mapping into the provider helper.
...
5) **Testing**  
   - Add Vitest suites under `frontend/src/domain/flights/__tests__/` covering mappers (Duffel happy/edge cases) and schema guards.  
   - Extend existing tool tests (`frontend/src/ai/tools/server/__tests__/flights*.test.ts`) to assert normalized `FlightOffer` structure and error mapping.
...
- Minor refactor required in `ai/tools/server/flights.ts` and any caller of `@schemas/agents` flight shapes; reduces long-term maintenance and drift.
