...
1. **Implementing a production-grade Expedia client** in `frontend/src/lib/travel-api/expedia-client.ts` with Rapid authentication (key + secret → Basic/OAuth per Rapid docs), request signing, retries, structured errors, and telemetry spans. The client will expose Content, Shopping, Booking, and cancellation helpers and reuse a singleton pattern behind `getExpediaClient()`.
2. **Wiring AI tools** in `frontend/src/lib/tools/accommodations.ts` to call the real client for search, details, availability, and booking flows, returning normalized DTOs. Tools will retain existing guardrails (cache, rate limit, telemetry) and propagate Rapid response metadata for UI rendering.
