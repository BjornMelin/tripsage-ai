...
2. **Wiring AI tools** in `frontend/src/ai/tools/server/accommodations.ts` to call the real client for search, details, availability, and booking flows, returning normalized DTOs. Tools retain existing guardrails (cache, rate limit, telemetry) and propagate Rapid response metadata for UI rendering.
3. **Coordinating payment + booking** in `frontend/src/lib/payments/booking-payment.ts`: charge via Stripe first (per ADR-0031), then call Rapid Booking. Store confirmations in Supabase and emit telemetry events for monitoring.
