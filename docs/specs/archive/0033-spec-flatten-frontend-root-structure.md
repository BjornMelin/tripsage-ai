...
- Risk: Documentation drift. We need to update internal docs to reflect new structure. For example, the [Development Guide](../development/core/development-guide.md#next-js-api-routes) likely says `cd frontend`. We must update that. Also, any code snippets in docs pointing to legacy frontend-prefixed source paths need updating to `src/...`.
