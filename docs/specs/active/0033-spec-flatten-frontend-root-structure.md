...
- CI scripts and caching: The CI integration with caching, keyed on lockfile and config files, needs path update. For example, the cache key should hash `pnpm-lock.yaml`, `tsconfig.json`, etc.,. After flattening, it should hash `pnpm-lock.yaml` and `tsconfig.json` at root. Similarly, artifact upload/download paths, such as `frontend/.vitest` or `frontend/coverage`, should change to `.vitest` and `coverage`.
...
- Risk: Documentation drift. We need to update internal docs to reflect new structure. For example, `docs/development/quick-start.md` likely says `cd frontend`. We must update that. Also, any code snippets in docs pointing to legacy frontend-prefixed source paths need updating to `src/...`.
