...
- **AI SDK Integration**: Use `streamText` / `generateText` with `Output.object({ schema })` for structured output (Zod schemas in `src/schemas`). UI hooks use `useChat`/`useAssistant` with `DefaultChatTransport`. No custom streaming stacks.
