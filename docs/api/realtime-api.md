...
- All channel lifecycles flow through `use-realtime-channel`. Each channel registers in `useRealtimeConnectionStore`, which records status (`connecting/subscribed/error/closed`), last activity, and errors.
