...
|-------|------|----------|-------------|
| `items` | array | Yes | Array of calendar items (min 1) |
| `timeMin` | string/date | Yes | Start time |
| `timeMax` | string/date | Yes | End time |
...
`200 OK` - Returns free/busy response
...
```bash
curl -X POST "http://localhost:3000/api/calendar/ics/export" \
...
| `validateOnly` | boolean | No | Validate only, don't import (default: true) |

#### Response

`200 OK` - Returns import result
