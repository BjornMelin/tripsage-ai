...
- **Input Schema (Zod for JS FastMCP, or Pydantic for Python FastMCP)**:
  ...
  z.object({
    origin: z
      .string()
      .length(3)
      .describe("Origin airport IATA code (e.g., 'LAX')"),
    destination: z
      .string()
      .length(3)
    ...
    max_connections: z
      ...
      .min(0)
      .nullish()
      .describe("Max connections per slice"),
    airline_codes: z
      .array(z.string())
      .default([])
      .describe("Preferred airline IATA codes"),
    currency: z
      .string()
      .length(3)
      .default("USD")
      .describe("Currency for prices (ISO 4217)"),
  });
  ```
...
- **Input Schema**:
  ...
  z.object({
    slices: z
      ...
      .min(2)
      .describe("List of flight slices for the multi-city trip"),
...
- **Input Schema**:
  ...
  z.object({
    selected_offers: z
      .array(z.string())
      .min(1)
      .describe("List of offer IDs to book"),
    passengers: z.array(
      z.object({
        // Detailed passenger schema
...
class DuffelService {
  ...
  async createOfferRequest(params) {
    try {
      ...
      logger.error("Duffel: Error creating offer request", {
        error: error.message,
  ...
  async getOffers(offerRequestId, limit = 50) {
    try {
      ...
      const offers = await this.duffel.offers.list(
        { offer_request_id: offerRequestId, limit },
        { headers: this.defaultHeaders }
      );
      logger.info(`Duffel: Retrieved ${offers.data.length} offers`);
      return offers.data;
    } catch (error) {
      logger.error("Duffel: Error getting offers", {
        error: error.message,
        details: error.response?.data,
      });
  ...
  async getFareRules(offerId) {
    try {
      logger.debug("Duffel: Getting offer conditions (fare rules)", {
        offerId,
      });
      ...
      return offer.data.conditions;
    } catch (error) {
      logger.error("Duffel: Error getting offer conditions", {
        error: error.message,
        details: error.response?.data,
      });
...
Functions for converting raw Duffel API responses into TripSage flight data models:

- `formatSearchResults(duffelOffers, currency)`
- `formatOffer(duffelOffer, airlinesRef, currency)`
- `formatSlice(duffelSlice, airlinesRef)`
- `formatSegment(duffelSegment, airlinesRef)`
- `extractAirlines(duffelOffers)`
- `formatOfferDetails(duffelOffer, duffelFareRules, currency)`
...
- **TTL Strategy**:
  ...
  - Offer Details: 2-5 minutes
  - Fare Rules: 60 minutes
...
- **Kubernetes**: Manifests for deploying to staging/production clusters.
- **Environment Variables**: `DUFFEL_API_KEY`, `REDIS_URL`, `PORT`, `LOG_LEVEL`.
- **Health Check**: A `/health` endpoint in the FastMCP server for Kubernetes liveness/readiness probes.

## 10. Testing

- **Unit Tests**: For `duffel_service.js`, `duffel_transformer.js`, and individual tool handlers.
...
- **End-to-End Tests**: (Limited due to external API dependency) Basic search tests against Duffel's test environment.

## 11. Monitoring and Logging

- **Logging**: Use a structured logger (e.g., Winston) to log requests, responses, errors, and cache interactions.
- **Metrics**: Expose Prometheus metrics for request counts, latencies, error rates, cache hit/miss.
- **Alerts**: High error rates from Duffel API or high MCP server error rates.
