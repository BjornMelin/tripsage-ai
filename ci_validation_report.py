...
def generate_validation_report():
    ...
    for filename in os.listdir(workflow_dir):
        if filename.endswith(".yml") or filename.endswith(".yaml"):
            try:
                ...
                validation_results["syntax_validation"].append(
                    {"file": filename, "status": "pass", "message": "Valid YAML"}
                )

            except yaml.YAMLError as e:
                print(f"❌ {filename} - YAML syntax error: {e}")
                validation_results["syntax_validation"].append(
                    {"file": filename, "status": "fail", "message": f"YAML error: {e}"}
                )
            except Exception as e:
                print(f"❌ {filename} - Error: {e}")
                validation_results["syntax_validation"].append(
                    {"file": filename, "status": "fail", "message": f"Error: {e}"}
                )
    ...
    if backend_branches != frontend_branches:
        ...
        print("⚠️  Branch trigger mismatch:")
    ...
    if quality_gates_workflow:
        ...
        if dependent_workflows:
            ...
            validation_results["dependency_validation"].append(
                {"status": "warning", "message": "No workflow dependencies configured"}
            )
    else:
        print("❌ No Quality Gates workflow found")
        validation_results["dependency_validation"].append(
            {"status": "fail", "message": "No quality gates workflow"}
        )
    ...
    for filename, info in workflows.items():
        if "security" in info["name"].lower():
            ...
            for job in info["jobs"]:
                if any(
                    keyword in job.lower()
                    for keyword in ["secret", "vulnerability", "audit", "scan"]
                ):
    ...
    if security_workflows:
        ...
        validation_results["security_validation"].append(
            {"status": "fail", "message": "No security workflows"}
        )
    ...
    for filename, info in workflows.items():
        ...
        for job in info["jobs"]:
            if any(
                keyword in job.lower()
                for keyword in ["quality", "lint", "test", "coverage"]
            ):
    ...
    syntax_passed = len(
        [r for r in validation_results["syntax_validation"] if r["status"] == "pass"]
    )
    ...
    if validation_results["trigger_validation"]:
        print(
            f"🎯 Trigger Issues: {len(validation_results['trigger_validation'])} found"
        )
