...
class SecurityAuditLogger:
    ...
    async def _write_event(self, event: AuditEvent):
        ...
        try:
            ...
            if (self.config.external_forwarding_enabled and 
    ...
    def _handle_circuit_breaker_failure(self):
        ...
        if (self._circuit_breaker_open and 
            datetime.now().timestamp() >= self._circuit_breaker_next_attempt):
            self._circuit_breaker_open = False
            self._circuit_breaker_failures = 0
...
async def audit_security_event(
    ...
    **metadata
    ...
    return await audit_logger.log_security_event(
        event_type, severity, message, actor_id, ip_address,
        target_resource, risk_score, metadata
    )
