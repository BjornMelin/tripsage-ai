...
from tripsage_core.services.business.audit_logging_service import (
    ...
    audit_security_event, get_audit_logger
)
...
class SecurityMonitoringService:
    ...
    def _initialize_patterns(self) -> List[ActivityPattern]:
        ...
        return [
            ...
            ActivityPattern(
                ...
                event_types=[AuditEventType.PERMISSION_CHANGED, AuditEventType.ROLE_ASSIGNED],
                time_window_minutes=15,
                min_occurrences=3,
                alert_threshold=0.9,
    ...
    async def _check_pattern(self, event: AuditEvent, pattern: ActivityPattern) -> Optional[ThreatIndicator]:
        ...
        if pattern.same_actor:
            actor_events = [e for e in self._actor_activity[event.actor.actor_id] 
                           if e.timestamp >= cutoff_time and e.event_type in pattern.event_types]
            relevant_events.extend(actor_events)
        
        if pattern.same_ip:
            ip_events = [e for e in self._ip_activity[event.source.ip_address] 
                        if e.timestamp >= cutoff_time and e.event_type in pattern.event_types]
        ...
        if pattern.same_service and event.target:
            ...
            if service:
                service_events = [e for e in self._service_activity[service] 
                                if e.timestamp >= cutoff_time and e.event_type in pattern.event_types]
        ...
        if len(relevant_events) >= pattern.min_occurrences:
            ...
            if len(relevant_events) > 1:
                time_spread = (max(e.timestamp for e in relevant_events) - 
                             min(e.timestamp for e in relevant_events)).total_seconds()
    ...
    async def _find_or_create_incident(self, threat: ThreatIndicator) -> SecurityIncident:
        ...
        for incident in self._active_incidents.values():
            if (threat.threat_category in incident.threat_categories and
                any(entity in incident.affected_users + incident.affected_ips 
    ...
    async def _take_automated_action(self, threat: ThreatIndicator, incident: SecurityIncident):
        ...
        if threat.threat_category in [ThreatCategory.BRUTE_FORCE, ThreatCategory.API_ABUSE]:
            for entity in threat.affected_entities:
    ...
    async def _cleanup_old_data(self):
        ...
        old_indicators = [
            tid for tid, threat in self._threat_indicators.items()
