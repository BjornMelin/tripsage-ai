...
class PrometheusMetrics:
    """Consolidated Prometheus metrics for database monitoring."""
    ...
    def __init__(self, registry=None):
        ...
        self.security_alerts = Counter(
            "tripsage_db_security_alerts_total",
...
class ConsolidatedDatabaseMonitor:
    ...
    def __init__(
        ...
        # Initialize metrics
        self.metrics = PrometheusMetrics(registry=metrics_registry) if self.config.metrics_enabled else None
    ...
    async def _perform_health_check(self) -> HealthCheckResult:
        ...
        # Trim history
        if len(self._health_history) > 100:  # Keep last 100 health checks
    ...
    async def _check_query_patterns(self):
        ...
        if len(slow_queries) > 10:  # More than 10 slow queries in recent history
            alert = SecurityAlert(
    ...
    async def _trigger_alert(self, alert: SecurityAlert):
        ...
        # Update metrics
        if self.metrics:
            self.metrics.security_alerts.labels(
                service="supabase",
    ...
    async def finish_query(
        ...
        if self.metrics:
            ...
            if not execution.is_successful and error_message:
