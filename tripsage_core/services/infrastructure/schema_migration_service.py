...
class SchemaMigrationService:
    ...
    async def check_migration_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.migration_status = {
                ...
                'migration_complete': self._assess_migration_complete(columns_check, uuid_status, mem0_check)
            }
    ...
    async def execute_schema_migration(self) -> Dict[str, Any]:
        ...
        try:
            ...
            migration_sql_path = "supabase/migrations/20250611_02_schema_alignment_fixes.sql"
            ...
            if status.get('migration_complete'):
                ...
                return {'success': False, 'status': status, 'warning': 'Migration may be incomplete'}
    ...
    async def migrate_trip_ids_to_uuid(self, batch_size: int = 100) -> Dict[str, Any]:
        ...
        """
        try:
            logger.info(f"Starting trip ID migration to UUID (batch size: {batch_size})")
            ...
            if not trips_to_migrate:
                ...
                return {'success': True, 'migrated': 0, 'message': 'All trips already have UUIDs'}
    ...
    async def setup_mem0_collections(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for collection_config in default_collections:
                ...
                if existing:
                    logger.debug(f"Collection {collection_config['name']} already exists")
                    continue
    ...
    async def validate_schema_integrity(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if validation_results['orphaned_flights'] > 0:
                integrity_issues.append(f"{validation_results['orphaned_flights']} orphaned flight records")
    ...
    async def _update_related_trip_references(self, trip_bigint_id: int, trip_uuid: str):
    ...
    def _assess_migration_complete(self, columns_check, uuid_status, mem0_check) -> bool:
        ...
        trips_status = next((row for row in uuid_status if row['table_name'] == 'trips'), None)
        if trips_status and trips_status['total_records'] != trips_status['uuid_populated']:
            return False
