"""
Database Monitoring for TripSage Core.
...
This is the unified, modern database monitoring solution that consolidates
all monitoring functionality into a single, maintainable module.
"""
...
class ConsolidatedDatabaseMonitor:
    ...
    async def _monitor_loop(self):
        ...
        while self._monitoring:
            try:
                ...
                # Health check
                if (
                    self.config.health_check_enabled
                    and current_time - last_health_check
                ...
                if (
                    ...
                    and current_time - last_security_check
                    >= self.config.security_check_interval
                ):
    ...
    async def _perform_health_check(self) -> HealthCheckResult:
        ...
        try:
            ...
            if is_healthy:
                ...
                status_msg = (
                    "passed" if status == HealthStatus.HEALTHY else "has concerns"
                )
                result = HealthCheckResult(
                    status=status,
                    response_time=response_time,
                    message=f"Database health check {status_msg}",
                    details=details,
                )
            else:
                result = HealthCheckResult(
    ...
    def _determine_health_status(
        self, details: Dict[str, Any], response_time: float
    ) -> HealthStatus:
    ...
    async def _check_query_patterns(self):
        ...
        slow_queries = [
            q for q in recent_queries if q.is_slow(self.config.slow_query_threshold)
        ]
            ...
            try:
                ...
                if self.database_service.is_connected:
                    ...
                    alert = SecurityAlert(
                        event_type=SecurityEvent.CONNECTION_FAILURE,
                        severity="info",
                        message=f"Database connection recovered after {attempts} attempts",  # noqa: E501
                ...
                if attempt == self.config.max_recovery_attempts - 1:
                    alert = SecurityAlert(
                        ...
                        message=(
                            ...
                            f"{self.config.max_recovery_attempts} attempts"
                        ),
                        details={
    ...
    async def _trigger_alert(self, alert: SecurityAlert):
        ...
        if len(self._security_alerts) > self.config.max_security_history:
            self._security_alerts = self._security_alerts[
                -self.config.max_security_history :
            ]
