...
class PerformanceRegressionDetector:
    ...
    def _analyze_real_time(self, metric_name: str, data_point: PerformanceDataPoint):
        ...
        is_regression = baseline.is_regression(data_point.value, self.regression_threshold)
    ...
    def _analyze_trend(self, metric_name: str):
        ...
        cutoff_time = datetime.now(timezone.utc) - timedelta(minutes=self.trend_window_minutes)
        recent_points = [
            dp for dp in data_points 
            if dp.timestamp >= cutoff_time
        ]
        ...
        timestamps = [(dp.timestamp - recent_points[0].timestamp).total_seconds() for dp in recent_points]
        values = [dp.value for dp in recent_points]
    ...
    def _generate_alert_message(
        ...
        if is_regression:
            parts.append(f"Performance regression detected")
        
        if is_anomaly:
            parts.append(f"Statistical anomaly detected")
    ...
    def _cleanup_old_data(self):
        ...
        cutoff_time = datetime.now(timezone.utc) - timedelta(hours=self.baseline_window_hours * 2)
    ...
    def get_metrics_summary(self) -> Dict[str, Any]:
        ...
        critical_alerts = len([a for a in self._alerts if a.severity == RegressionSeverity.CRITICAL])
