...
class ComplianceReport(TripSageModel):
    ...
    report_id: str = Field(default_factory=lambda: f"rpt-{int(datetime.now().timestamp())}")
...
class AuditQueryEngine:
    ...
    async def _execute_query(self, query: AuditQuery) -> List[AuditEvent]:
        ...
        events = await self.audit_logger.query_events(
            ...
            severity=query.severities[0] if query.severities and len(query.severities) == 1 else None,
            actor_id=query.actor_ids[0] if query.actor_ids and len(query.actor_ids) == 1 else None,
            limit=query.limit,
        )
    ...
    def _matches_filters(self, event: AuditEvent, query: AuditQuery) -> bool:
        ...
        if query.min_risk_score is not None:
            if not event.risk_score or event.risk_score < query.min_risk_score:
                return False
    ...
    async def _analyze_events(self, events: List[AuditEvent], query: AuditQuery) -> AuditAnalysisResult:
        ...
        # Event type analysis
        for event in events:
            result.events_by_type[event.event_type.value] = result.events_by_type.get(event.event_type.value, 0) + 1
            result.events_by_severity[event.severity.value] = result.events_by_severity.get(event.severity.value, 0) + 1
            result.events_by_outcome[event.outcome.value] = result.events_by_outcome.get(event.outcome.value, 0) + 1
            result.events_by_actor[event.actor.actor_id] = result.events_by_actor.get(event.actor.actor_id, 0) + 1
            result.events_by_ip[event.source.ip_address] = result.events_by_ip.get(event.source.ip_address, 0) + 1
    ...
    async def generate_compliance_report(
        ...
        report.compliance_violations = await self._analyze_compliance_violations(analysis.events, report_type)
        
        # Analyze policy violations
        report.policy_violations = await self._analyze_policy_violations(analysis.events)
        
        # Calculate overall risk score
        report.overall_risk_score = await self._calculate_overall_risk_score(analysis)
        
        # Generate recommendations
        report.recommendations = await self._generate_recommendations(analysis, report_type)
    ...
    async def _analyze_compliance_violations(self, events: List[AuditEvent], report_type: str) -> List[Dict[str, Any]]:
        ...
        if report_type.upper() == "HIPAA":
            ...
            for event in events:
                if (event.event_type == AuditEventType.DATA_ACCESS and 
                    not event.metadata.get("hipaa_authorized", False)):
                    violations.append({
            ...
            # SOX-specific checks
            for event in events:
                if (event.event_type == AuditEventType.CONFIG_CHANGED and
                    event.severity in [AuditSeverity.HIGH, AuditSeverity.CRITICAL] and
                    not event.metadata.get("sox_approved", False)):
                    violations.append({
    ...
    async def _analyze_policy_violations(self, events: List[AuditEvent]) -> List[Dict[str, Any]]:
    ...
    async def _generate_recommendations(self, analysis: AuditAnalysisResult, report_type: str) -> List[str]:
        ...
        ip_counts = sorted(analysis.events_by_ip.items(), key=lambda x: x[1], reverse=True)
