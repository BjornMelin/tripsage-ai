...
async def validate_file(file: UploadFile, max_size: int = MAX_FILE_SIZE) -> ValidationResult:
    """
    ...
    # Size validation
    if file_size == 0:
        return ValidationResult(is_valid=False, error_message="File is empty", file_size=file_size)
    ...
    if not file.filename:
        return ValidationResult(is_valid=False, error_message="Filename is required", file_size=file_size)

    filename_result = _validate_filename(file.filename)
    if not filename_result[0]:
        return ValidationResult(is_valid=False, error_message=filename_result[1], file_size=file_size)
...
def _validate_file_content(content: bytes, mime_type: str) -> Tuple[bool, Optional[str]]:
    """
...
def _validate_image_content(content: bytes, mime_type: str) -> Tuple[bool, Optional[str]]:
...
async def validate_batch_upload(files: list[UploadFile], max_total_size: int = MAX_SESSION_SIZE) -> ValidationResult:
    """
    ...
    """
    if not files:
        return ValidationResult(is_valid=False, error_message="No files provided", file_size=0)
    ...
    if total_size > max_total_size:
        return ValidationResult(
            is_valid=False,
            error_message=(f"Total batch size ({total_size} bytes) exceeds maximum ({max_total_size} bytes)"),
