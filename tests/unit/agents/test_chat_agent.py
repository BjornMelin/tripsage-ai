...
class TestChatAgent:
    ...
    def chat_agent(self, mock_specialized_agents):
        ...
        with (
            ...
            patch(
                ...
                return_value=mock_specialized_agents["destination_agent"],
            ),
            patch(
                "tripsage.agents.chat.ItineraryAgent",
                return_value=mock_specialized_agents["itinerary_agent"],
            ),
            patch(
                "tripsage.agents.chat.TravelAgent",
                return_value=mock_specialized_agents["travel_agent"],
            ),
    ...
    def test_initialization_with_custom_params(self):
        ...
        with (
            ...
            agent = ChatAgent(
                name="Custom Chat Agent", model="gpt-3.5-turbo", temperature=0.5
            )
    ...
    def test_initialization_agent_failure(self):
        ...
        with (
            ...
            assert "Failed to initialize specialized agents" in str(
    ...
    async def test_detect_intent_confidence_levels(self, chat_agent):
        ...
        high_confidence_intent = await chat_agent.detect_intent(
            "Book a flight ticket from airport"
        )
        ...
        low_confidence_intent = await chat_agent.detect_intent(
            "I mentioned flight in passing"
        )
    ...
    async def test_process_message_routing(self, chat_agent, mock_specialized_agents):
        ...
        message = (
            "Find flights to Paris departure tomorrow"  # High confidence flight intent
        )
        context = {"user_id": "test_user"}
    ...
    async def test_run_with_tools(self, chat_agent):
        ...
        with patch.object(
            chat_agent, "process_message", new_callable=AsyncMock
        ) as mock_process:
