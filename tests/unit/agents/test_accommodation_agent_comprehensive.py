...
class TestAccommodationAgent:
    ...
    def test_register_accommodation_tools_partial_failure(self, accommodation_agent):
        ...
        with patch.object(accommodation_agent, "register_tool_group", side_effect=mock_register_side_effect):
    ...
    def test_instructions_include_search_parameters(self, accommodation_agent):
        ...
        for param in search_params:
            assert param in instructions, f"Search parameter '{param}' not found in instructions"
    ...
    def test_instructions_include_presentation_format(self, accommodation_agent):
        ...
        for element in presentation_elements:
            assert element in instructions, f"Presentation element '{element}' not found in instructions"
    ...
    def test_instructions_include_booking_guidance(self, accommodation_agent):
        ...
        for concept in booking_concepts:
            assert concept in instructions, f"Booking concept '{concept}' not found in instructions"
    ...
    def test_agent_specialization_focus(self, accommodation_agent):
        ...
        found_types = sum(1 for acc_type in accommodation_types if acc_type in instructions.lower())
    ...
    def test_tool_module_coverage(self, accommodation_agent):
        ...
        with patch.object(accommodation_agent, "register_tool_group") as mock_register:
            ...
            for module in expected_modules:
                assert module in registered_modules, f"Tool module '{module}' not registered"
    ...
    def test_instructions_completeness(self, accommodation_agent):
        ...
        assert "responsibilities" in instructions.lower() or "guidelines" in instructions.lower()
    ...
    @pytest.mark.asyncio
    async def test_accommodation_workflow_simulation(self, accommodation_agent, mock_openai_runner):
        ...
        assert "accommodation" in result["content"].lower() or "hotel" in result["content"].lower()
    ...
    def test_tool_registration_resilience(self):
        ...
        with patch("agents.Agent") as mock_agent_cls:
            with patch("agents.Runner") as mock_runner_cls:
                ...
                with patch.object(agent, "register_tool_group", side_effect=Exception("Tool registration failed")):
    ...
    def test_settings_fallback(self, mock_openai_agent, mock_openai_runner):
        ...
        with patch("agents.Agent", return_value=mock_openai_agent):
            with patch("agents.Runner", return_value=mock_openai_runner):
                ...
                with patch("tripsage.agents.accommodation.settings", side_effect=AttributeError("Settings not available")):
