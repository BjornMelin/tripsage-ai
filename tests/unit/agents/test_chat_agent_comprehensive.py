...
class TestChatAgent:
    ...
    def mock_specialized_agents(self):
        ...
        agents = {}
        agent_types = ["flight", "accommodation", "budget", "destination", "itinerary", "travel"]
    ...
    def mock_chat_service(self):
        ...
        service = MagicMock()
        service.create_chat_session = AsyncMock(return_value={
            "session_id": "session_123",
            "user_id": 1,
            "status": "active"
        })
        service.save_message = AsyncMock(return_value={
            "message_id": "msg_123",
            "session_id": "session_123"
        })
        service.execute_parallel_tools = AsyncMock(return_value={
            "total_calls": 1,
            "success_count": 1,
    ...
    @pytest.fixture
    def chat_agent(self, mock_mcp_manager, mock_memory_service, mock_chat_service, mock_specialized_agents):
        ...
        with patch("tripsage.agents.chat.MCPManager", return_value=mock_mcp_manager):
            with patch("tripsage.agents.chat.ChatOrchestrationService", return_value=mock_chat_service):
                with patch("tripsage.agents.chat.TripSageMemoryService", return_value=mock_memory_service):
                    with patch("tripsage.agents.chat.FlightAgent", return_value=mock_specialized_agents["flight_agent"]):
                        with patch("tripsage.agents.chat.AccommodationAgent", return_value=mock_specialized_agents["accommodation_agent"]):
                            with patch("tripsage.agents.chat.BudgetAgent", return_value=mock_specialized_agents["budget_agent"]):
                                with patch("tripsage.agents.chat.DestinationResearchAgent", return_value=mock_specialized_agents["destination_agent"]):
                                    with patch("tripsage.agents.chat.ItineraryAgent", return_value=mock_specialized_agents["itinerary_agent"]):
                                        with patch("tripsage.agents.chat.TravelAgent", return_value=mock_specialized_agents["travel_agent"]):
                                            agent = ChatAgent()
                                            # Set up the specialized agents
                                            for agent_type, mock_agent in mock_specialized_agents.items():
                    ...
                    with patch("tripsage.agents.chat.FlightAgent"):
                        with patch("tripsage.agents.chat.AccommodationAgent"):
                            with patch("tripsage.agents.chat.BudgetAgent"):
                                with patch("tripsage.agents.chat.DestinationResearchAgent"):
    ...
    def test_initialize_specialized_agents_failure(self):
        ...
        with patch("tripsage.agents.chat.MCPManager"):
            with patch("tripsage.agents.chat.ChatOrchestrationService"):
                with patch("tripsage.agents.chat.TripSageMemoryService"):
                    with patch("tripsage.agents.chat.FlightAgent", side_effect=Exception("Agent init failed")):
                        with patch("tripsage.agents.chat.AccommodationAgent"):
                            with patch("tripsage.agents.chat.BudgetAgent"):
                                with patch("tripsage.agents.chat.DestinationResearchAgent"):
                                    with patch("tripsage.agents.chat.ItineraryAgent"):
    ...
    def test_register_travel_tools_failure(self, chat_agent):
        ...
        with patch.object(chat_agent, "register_tool_group", side_effect=Exception("Tool registration failed")):
            # Should not raise exception
            chat_agent._register_travel_tools()

    @pytest.mark.asyncio
    async def test_ensure_memory_initialized_success(self, chat_agent, mock_memory_service):
    ...
    @pytest.mark.asyncio
    async def test_ensure_memory_initialized_already_done(self, chat_agent, mock_memory_service):
    ...
    @pytest.mark.asyncio
    async def test_ensure_memory_initialized_failure(self, chat_agent, mock_memory_service):
    ...
    @pytest.mark.asyncio
    async def test_store_conversation_memory_success(self, chat_agent, mock_memory_service):
    ...
    @pytest.mark.asyncio
    async def test_store_conversation_memory_failure(self, chat_agent, mock_memory_service):
    ...
    async def test_route_to_agent_budget(self, chat_agent, mock_specialized_agents):
        ...
        mock_specialized_agents["budget_agent"].run.assert_called_once_with(message, context)

    @pytest.mark.asyncio
    async def test_route_to_agent_fallback_to_travel(self, chat_agent, mock_specialized_agents):
    ...
    async def test_route_to_agent_error_handling(self, chat_agent, mock_specialized_agents):
        ...
        mock_specialized_agents["flight_agent"].run.side_effect = Exception("Agent failed")
    ...
    @pytest.mark.asyncio
    async def test_process_message_with_routing(self, chat_agent, mock_specialized_agents, mock_chat_service, mock_memory_service):
    ...
    @pytest.mark.asyncio
    async def test_process_message_direct_handling(self, chat_agent, mock_chat_service, mock_memory_service):
    ...
    @pytest.mark.asyncio
    async def test_process_message_session_creation(self, chat_agent, mock_chat_service, mock_memory_service):
    ...
    async def test_route_request_failure(self, chat_agent):
        ...
        with patch.object(chat_agent, "detect_intent", side_effect=Exception("Intent detection failed")):
    ...
    async def test_call_mcp_tools_failure(self, chat_agent, mock_chat_service):
        ...
        chat_agent.chat_service = mock_chat_service
        mock_chat_service.execute_parallel_tools.side_effect = Exception("Tool execution failed")
    ...
    async def test_create_chat_session_mcp_success(self, chat_agent, mock_chat_service):
        ...
        mock_chat_service.create_chat_session.assert_called_once_with(123, {"source": "test"})

    @pytest.mark.asyncio
    async def test_create_chat_session_mcp_failure(self, chat_agent, mock_chat_service):
        """Test MCP chat session creation failure."""
        chat_agent.chat_service = mock_chat_service
        mock_chat_service.create_chat_session.side_effect = Exception("Session creation failed")
    ...
    async def test_get_chat_history_mcp_success(self, chat_agent, mock_chat_service):
        ...
        result = await chat_agent.get_chat_history_mcp("session_123", limit=10, offset=0)
    ...
    async def test_get_chat_history_mcp_failure(self, chat_agent, mock_chat_service):
        ...
        chat_agent.chat_service = mock_chat_service
        mock_chat_service.get_chat_history.side_effect = Exception("History retrieval failed")
...
class TestChatAgentErrorHandling:
        ...
        with patch("tripsage.agents.chat.MCPManager"):
            with patch("tripsage.agents.chat.ChatOrchestrationService"):
                with patch("tripsage.agents.chat.TripSageMemoryService"):
                    ...
                    with patch("tripsage.agents.chat.FlightAgent", side_effect=Exception("Agent init failed")):
                        with patch("tripsage.agents.chat.AccommodationAgent", side_effect=Exception("Agent init failed")):
                            with patch("tripsage.agents.chat.BudgetAgent", side_effect=Exception("Agent init failed")):
                                with patch("tripsage.agents.chat.DestinationResearchAgent", side_effect=Exception("Agent init failed")):
                                    with patch("tripsage.agents.chat.ItineraryAgent", side_effect=Exception("Agent init failed")):
                                        with patch("tripsage.agents.chat.TravelAgent", side_effect=Exception("Agent init failed")):
                                            return ChatAgent()
    ...
    async def test_route_to_agent_missing_agent(self, minimal_chat_agent):
        ...
        with patch("tripsage.agents.base.BaseAgent.run") as mock_run:
            mock_run.return_value = {"content": "Fallback response", "status": "success"}
