...
class TestAccommodationAgent:
    ...
    def test_tool_registration(self):
        ...
        with patch("agents.Agent"):
            with patch.object(
                AccommodationAgent, "register_tool_group"
            ) as mock_register:
                agent = AccommodationAgent(service_registry=registry)
    ...
    def test_agent_specialization_focus(self):
        ...
        with patch("agents.Agent"):
            ...
            instructions = agent.instructions.lower()
            assert (
                instructions.count("accommodation") >= 2
            )  # Should mention multiple times
    ...
    def test_service_registry_integration(self):
        ...
        with patch("agents.Agent"):
            ...
            assert (
                agent.service_registry.get_optional_service("accommodation_service")
                is mock_accommodation_service
            )
            assert (
                agent.service_registry.get_optional_service("memory_service")
                is mock_memory_service
            )
...
class TestAccommodationAgentEdgeCases:
    ...
    def test_memory_integration_capability(self):
        ...
        with patch("agents.Agent"):
            ...
            memory_service = agent.service_registry.get_optional_service(
                "memory_service"
            )
    ...
    def test_accommodation_service_integration(self):
        ...
        with patch("agents.Agent"):
            ...
            accommodation_service = agent.service_registry.get_optional_service(
