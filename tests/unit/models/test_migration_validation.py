...
class TestMigrationScriptValidation:
    ...
    def test_migration_file_structure(self, migration_sql):
        ...
        assert any("-- Description:" in line for line in lines[:10]), "Missing description comment"
        assert any("-- Issue: BJO-121" in line for line in lines[:10]), "Missing issue reference"
        assert any("-- Priority: HIGH" in line for line in lines[:10]), "Missing priority indicator"
    ...
    def test_migration_sql_syntax_structure(self, migration_sql):
        ...
        assert begin_count == commit_count == 1, (
            f"Expected 1 BEGIN and 1 COMMIT, found {begin_count} BEGIN, {commit_count} COMMIT"
        )
        ...
        assert "ALTER TABLE" in transaction_content, "ALTER TABLE operations should be within transaction"
        assert "CREATE POLICY" in transaction_content, "Policy creation should be within transaction"
    ...
    def test_migration_safety_measures(self, migration_sql):
        ...
        assert "DROP CONSTRAINT" in migration_sql, "Should document constraint removal for rollback"

        # Check for verification queries
        assert "VERIFICATION QUERIES" in migration_sql, "Should include verification instructions"
    ...
    def test_migration_uuid_conversion_safety(self, migration_sql):
        ...
        assert uuid_regex_pattern in migration_sql, "Should validate UUID format before conversion"
    ...
    def test_migration_foreign_key_operations(self, migration_sql):
        ...
        for pattern in fk_patterns:
            assert re.search(pattern, migration_sql), f"Missing FK constraint pattern: {pattern}"

        # Should reference auth.users table correctly
        assert "REFERENCES auth.users(id)" in migration_sql, "Should reference auth.users(id)"
    ...
    def test_migration_rls_implementation(self, migration_sql):
        ...
        for pattern in rls_patterns:
            assert re.search(pattern, migration_sql), f"Missing RLS enablement: {pattern}"
        ...
        assert "auth.uid() = user_id" in migration_sql, "Should use auth.uid() for isolation"
    ...
    def test_migration_performance_considerations(self, migration_sql):
        ...
        for pattern in index_patterns:
            assert re.search(pattern, migration_sql), f"Missing performance index: {pattern}"
    ...
    def test_migration_data_integrity_checks(self, migration_sql):
        ...
        for pattern in verification_patterns:
            assert re.search(pattern, migration_sql), f"Missing integrity check: {pattern}"
    ...
    def test_migration_function_updates(self, migration_sql):
        ...
        assert "search_memories" in migration_sql, "Should reference search_memories function"
        assert "Updated search_memories function to use UUID parameter" in migration_sql, "Should log function update"
    ...
    def test_migration_error_handling_blocks(self, migration_sql):
        ...
        assert "auth.users table not found" in migration_sql, "Should verify auth schema"
    ...
    def test_migration_logging_and_traceability(self, migration_sql):
        ...
        assert "Starting migration: Fix user_id constraints" in migration_sql, "Should log migration start"
        assert "Migration completed successfully" in migration_sql, "Should log completion"
    ...
    def test_migration_system_user_handling(self, migration_sql):
        ...
        assert "system@tripsage.internal" in migration_sql, "Should create system user email"
    ...
    def test_migration_sql_best_practices(self, migration_sql):
        ...
        assert '"memories"' in migration_sql or "memories" in migration_sql, "Should handle table names properly"

        # Should include proper comments for complex operations
        comment_count = migration_sql.count("-- ")
        assert comment_count > 50, f"Should have comprehensive comments, found {comment_count}"
    ...
    def test_migration_rollback_completeness(self, migration_sql):
        ...
        for operation in rollback_operations:
            assert operation in rollback_section, f"Missing rollback operation: {operation}"
    ...
    def test_migration_idempotency_safety(self, migration_sql):
        ...
        assert if_not_exists_count >= 3, f"Should use IF NOT EXISTS for safety, found {if_not_exists_count}"
    ...
    def test_migration_performance_impact(self, migration_sql):
        ...
        assert "ALTER COLUMN user_id TYPE UUID USING" in migration_sql, "Should use efficient type conversion"
        ...
        if alter_table_pos > 0 and create_index_pos > 0:
            assert alter_table_pos < create_index_pos, "Indexes should be created after table alterations"
