...
class TestPGVectorServiceIndexCreation:
    ...
    @pytest.mark.asyncio
    async def test_create_hnsw_index_different_distance_functions(self, pgvector_service):
        ...
        for distance_func, ops_string, suffix in test_cases:
            ...
            call_args = pgvector_service.db.execute_sql.call_args_list[-2]  # -2 because _set_default_ef_search is also called
...
class TestPGVectorServiceQueryOptimization:
    ...
    async def test_set_query_quality(self, pgvector_service):
        ...
        # Assert
        pgvector_service.db.execute_sql.assert_called_with(f"SET hnsw.ef_search = {ef_search}")
    ...
    async def test_optimize_for_table_no_index(self, pgvector_service):
        ...
        with patch.object(pgvector_service, 'get_index_stats') as mock_get_stats:
            with patch.object(pgvector_service, 'create_hnsw_index') as mock_create:
                with patch.object(pgvector_service, 'set_query_quality') as mock_set_quality:
                    with patch.object(pgvector_service, 'check_index_health') as mock_health:
                        mock_get_stats.return_value = None
...
class TestPGVectorServiceTableDiscovery:
    ...
    async def test_list_vector_tables_with_results(self, pgvector_service):
        ...
        # Arrange
        mock_table_data = [
            {"table_name": "destinations", "column_name": "embedding", "index_status": "indexed"},
            {"table_name": "memories", "column_name": "embedding", "index_status": "no_index"},
            {"table_name": "conversations", "column_name": "vector_content", "index_status": "indexed"},
        ]
...
class TestPGVectorServiceUtilityFunctions:
    ...
    async def test_optimize_vector_table_utility(self, mock_database_service):
        ...
        with patch('tripsage_core.services.infrastructure.pgvector_service.PGVectorService') as MockService:
            mock_service_instance = Mock()
            mock_service_instance.optimize_for_table = AsyncMock(return_value={"test": "result"})
...
class TestPGVectorServiceErrorHandling:
    ...
    async def test_create_index_database_error(self, pgvector_service):
        ...
        # Arrange
        pgvector_service.db.execute_sql.side_effect = Exception("Database connection failed")
...
class TestPGVectorServicePropertyBased:
    ...
    @given(
        ...
        column_name=st.text(min_size=1, max_size=50, alphabet=st.characters(whitelist_categories=['Ll', 'Lu', 'Nd']))
    )
    @pytest.mark.asyncio
    async def test_index_name_generation_property(self, pgvector_service, table_name, column_name):
        ...
        with patch.object(pgvector_service, 'get_index_stats') as mock_get_stats:
            with patch.object(pgvector_service, 'create_hnsw_index') as mock_create:
                with patch.object(pgvector_service, 'check_index_health') as mock_health:
                    ...
                    assert execution_time < 3.0  # Should handle bulk operations efficiently
...
class TestPGVectorServiceIntegration:
    ...
    async def test_complete_optimization_workflow(self, pgvector_service):
        ...
        vector_tables_data = [
            {"table_name": table_name, "column_name": column_name, "index_status": "no_index"}
        ]
                ...
                with patch.object(pgvector_service, 'get_index_stats') as mock_get_stats:
                    with patch.object(pgvector_service, 'check_index_health') as mock_health:
                        ...
                        mock_optimize.return_value = {
                            "table": table_name,
                            "column": column_name,
                            "actions": [{"action": "created_index", "index_name": "new_index"}]
                        }
                        mock_get_stats.return_value = mock_stats
    ...
    async def test_memory_optimization_integration(self, pgvector_service):
        ...
        with patch.object(pgvector_service, 'optimize_for_table') as mock_optimize:
            ...
            optimized_tables = [opt["table"] for opt in memory_results["memory_optimization"]]
