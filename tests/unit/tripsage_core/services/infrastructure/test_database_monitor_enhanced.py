...
class TestDatabaseMonitorInitialization:
    ...
    @pytest_asyncio.fixture
    async def database_monitor(self, mock_database_service, monitoring_config, mock_settings):
    ...
    def test_initialization_default(self, mock_database_service):
        ...
        # Arrange & Act
        with patch('tripsage_core.services.infrastructure.database_monitor.get_settings') as mock_get_settings:
    ...
    def test_initialization_with_config(self, mock_database_service, monitoring_config, mock_settings):
    ...
    def test_initialization_with_metrics(self, mock_database_service, mock_settings):
        ...
        with patch('tripsage_core.services.infrastructure.database_monitor.Counter') as mock_counter:
            with patch('tripsage_core.services.infrastructure.database_monitor.Gauge') as mock_gauge:
                with patch('tripsage_core.services.infrastructure.database_monitor.Histogram') as mock_histogram:
    ...
    def test_initialization_metrics_import_error(self, mock_database_service, mock_settings):
        """Test database monitor initialization when Prometheus is not available."""
        # Arrange
        config = MonitoringConfig(metrics_enabled=True)
        
        # Mock ImportError for prometheus_client
        with patch('tripsage_core.services.infrastructure.database_monitor.ConsolidatedDatabaseMonitor._initialize_metrics') as mock_init:
...
class TestDatabaseMonitorHealthChecks:
    ...
    async def test_health_check_exception_handling(self, database_monitor):
        ...
        # Arrange
        database_monitor.database_service.health_check.side_effect = Exception("Connection error")
...
class TestDatabaseMonitorQueryTracking:
    ...
    async def test_query_history_limiting(self, database_monitor):
        ...
        for i in range(10):
            query_id = await database_monitor.track_query(QueryType.SELECT, f"table_{i}")
    ...
    async def test_monitor_query_context_manager(self, database_monitor):
        ...
        # Arrange & Act
        async with database_monitor.monitor_query(QueryType.DELETE, "old_data") as query_id:
            assert query_id != ""
...
class TestDatabaseMonitorSecurityMonitoring:
    ...
    async def test_security_check_high_error_rate(self, database_monitor):
        ...
        for i in range(10):
            ...
            await database_monitor.finish_query(query_id, status, "Test error" if status == QueryStatus.ERROR else None)
    ...
    async def test_security_check_exception_handling(self, database_monitor):
        ...
        # Arrange
        with patch.object(database_monitor, '_check_query_patterns', side_effect=Exception("Test error")):
            # Act
            await database_monitor._perform_security_check()
...
class TestDatabaseMonitorRecovery:
    ...
    async def test_recovery_failure_all_attempts(self, database_monitor):
        ...
        # Arrange
        database_monitor.database_service.is_connected = False
        database_monitor.database_service.connect.side_effect = Exception("Connection failed")
        
        # Act
        await database_monitor._attempt_recovery()
        
        # Assert
        assert database_monitor.database_service.connect.call_count == database_monitor.config.max_recovery_attempts
        alerts = database_monitor.get_security_alerts()
        failure_alerts = [
            alert for alert in alerts
            if alert.event_type == SecurityEvent.CONNECTION_FAILURE
...
class TestDatabaseMonitorLifecycle:
    ...
    async def test_monitor_loop_exception_handling(self, database_monitor):
        ...
        # Arrange
        database_monitor.database_service.health_check.side_effect = Exception("Test error")
...
class TestDatabaseMonitorPropertyBased:
    ...
    @pytest.mark.asyncio
    async def test_error_rate_calculation_property(self, database_monitor, query_count, error_rate):
        ...
        for i in range(error_count):
            query_id = await database_monitor.track_query(QueryType.SELECT, "users")
            await database_monitor.finish_query(query_id, QueryStatus.ERROR, "Test error")
    ...
    async def test_concurrent_query_tracking_performance(self, database_monitor):
        ...
        for i in range(query_count):
            async def track_query_task(index):
                query_id = await database_monitor.track_query(QueryType.SELECT, f"table_{index}")
