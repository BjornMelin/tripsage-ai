...
class TestWebSocketManagerAuthentication:
    ...
    async def test_authenticate_connection_success(
        ...
        # Arrange
        user_id = uuid4()
        with patch.object(started_manager.auth_service, 'verify_jwt_token') as mock_verify:
            with patch.object(started_manager, '_check_connection_rate_limit') as mock_rate_limit:
                with patch.object(started_manager.auth_service, 'get_available_channels') as mock_channels:
                    with patch.object(started_manager.auth_service, 'validate_channel_access') as mock_validate:
                        mock_verify.return_value = user_id
                        mock_rate_limit.return_value = True
                        mock_channels.return_value = ["general", "notifications", "private"]
...
class TestWebSocketManagerChannelSubscription:
    ...
    async def test_subscribe_connection_success(self, started_manager_with_connection):
        ...
        subscribe_request = WebSocketSubscribeRequest(
            channels=["general", "notifications"],
            unsubscribe_channels=["old_channel"]
        )
        
        with patch.object(manager.auth_service, 'get_available_channels') as mock_channels:
            with patch.object(manager.auth_service, 'validate_channel_access') as mock_validate:
                with patch.object(manager.messaging_service, 'subscribe_to_channel') as mock_subscribe:
                    with patch.object(manager.messaging_service, 'unsubscribe_from_channel') as mock_unsubscribe:
                        mock_channels.return_value = ["general", "notifications", "private"]
                        mock_validate.return_value = (["general", "notifications"], [])
                        mock_subscribe.return_value = True
                        
                        # Act
                        response = await manager.subscribe_connection(connection_id, subscribe_request)
                        
                        # Assert
                        assert response.success
                        assert "general" in response.subscribed_channels
                        assert "notifications" in response.subscribed_channels
                        mock_unsubscribe.assert_called_with(connection_id, "old_channel")

    @pytest.mark.asyncio
    async def test_subscribe_connection_not_found(self, started_manager_with_connection):
    ...
    @pytest.mark.asyncio
    async def test_subscribe_connection_access_denied(self, started_manager_with_connection):
...
class TestWebSocketManagerMessaging:
    ...
    async def test_send_to_connection_success(self, manager_with_connections):
        ...
        with patch.object(manager.messaging_service, 'send_to_connection') as mock_send:
            ...
            mock_send.assert_called_once_with(connection_id, event, manager.rate_limiter)
...
class TestWebSocketManagerBackgroundTasks:
    ...
    async def test_cleanup_stale_connections(self, started_manager):
        ...
        with patch.object(started_manager.connection_service, 'get_stale_connections') as mock_stale:
            with patch.object(started_manager, 'disconnect_connection') as mock_disconnect:
...
class TestWebSocketManagerStatistics:
    ...
    def test_get_connection_stats(self, started_manager):
        ...
        with patch.object(started_manager.messaging_service, 'get_connection_stats') as mock_stats:
    ...
    async def test_performance_monitor_updates_metrics(self, started_manager):
        ...
        # Arrange
        initial_active = started_manager.performance_metrics["active_connections"]
...
class TestWebSocketManagerErrorHandling:
    ...
    async def test_disconnect_connection_error_handling(self, started_manager):
        ...
        with patch.object(started_manager.connection_service, 'remove_connection') as mock_remove:
...
class TestWebSocketManagerPerformance:
    ...
    async def test_concurrent_broadcasting_performance(self):
        ...
        try:
            ...
            assert execution_time < 1.0  # Should handle concurrent broadcasts efficiently
