...
class TestDatabaseServiceCore:
    ...
    async def mock_settings(self):
        ...
        settings.database_public_key.get_secret_value.return_value = "test-key-that-is-long-enough"
    ...
    async def test_select_basic(self, connected_service):
        ...
        with patch.object(connected_service, '_get_client_for_query') as mock_get_client:
            mock_get_client.return_value.__aenter__ = AsyncMock(return_value=("primary", mock_client))
    ...
    @pytest.mark.asyncio
    async def test_ensure_connected_when_not_connected(self, database_service, mock_supabase_client):
    ...
    @pytest.mark.asyncio
    async def test_operation_when_not_connected(self, database_service, mock_supabase_client):
...
class TestDatabaseServicePerformance:
    ...
    async def test_concurrent_operations(self, performance_service):
        ...
        with patch.object(performance_service, '_get_client_for_query') as mock_get_client:
            mock_get_client.return_value.__aenter__ = AsyncMock(return_value=("primary", mock_client))
            mock_get_client.return_value.__aexit__ = AsyncMock(return_value=None)
...
class TestDatabaseServicePropertyBased:
    ...
    async def simple_service(self):
        ...
        mock_settings.database_public_key.get_secret_value.return_value = "test-key-that-is-long-enough"
