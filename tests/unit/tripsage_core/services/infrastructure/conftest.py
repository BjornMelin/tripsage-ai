...
def edge_case_database_configs(draw):
    ...
    pool_config = DatabasePoolConfig(
        ...
        max_overflow=draw(st.sampled_from([0, 1, 2000])),  # Keep within validation limits
...
def database_service_factory(mock_settings_factory):
    ...
    def _create_service(config: Optional[DatabaseConfig] = None, **legacy_overrides):
        ...
        if config is None:
            # If no config provided but legacy overrides exist, create config from legacy parameters
            if legacy_overrides:
                ...
                monitoring_config = DatabaseMonitoringConfig(
                    ...
                    enable_query_tracking=legacy_overrides.get('enable_query_tracking', True),
                    slow_query_threshold=legacy_overrides.get('slow_query_threshold', 1.0)
                )
                
                security_config = DatabaseSecurityConfig(
                    enable_security=legacy_overrides.get('enable_security', True),
                    enable_rate_limiting=legacy_overrides.get('enable_rate_limiting', True),
                    enable_audit_logging=legacy_overrides.get('enable_audit_logging', True),
                    rate_limit_requests=legacy_overrides.get('rate_limit_requests', 1000),
                    rate_limit_burst=legacy_overrides.get('rate_limit_burst', 2000)
                )
                
                performance_config = DatabasePerformanceConfig(
                    enable_read_replicas=legacy_overrides.get('enable_read_replicas', True),
                    enable_circuit_breaker=legacy_overrides.get('enable_circuit_breaker', True),
                    circuit_breaker_threshold=legacy_overrides.get('circuit_breaker_threshold', 5),
                    circuit_breaker_timeout=legacy_overrides.get('circuit_breaker_timeout', 60.0)
                )
...
async def load_test_data():
    ...
    return {
        "users": users,
        "trips": trips,
        "query_vectors": [[0.1 * i] * 1536 for i in range(50)],  # For vector search tests
    }
...
def pytest_configure(config):
    ...
    config.addinivalue_line("markers", "chaos: Chaos engineering and error injection tests")
