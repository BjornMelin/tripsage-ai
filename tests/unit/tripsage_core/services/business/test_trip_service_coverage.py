...
class TestTripServiceCoverage:
    ...
    def trip_service(self, mock_database_service):
        ...
        service = TripService(
            database_service=mock_database_service, user_service=Mock()
        )
        return service

    @pytest.mark.asyncio
    async def test_get_trip_basic_functionality(
    ...
    @pytest.mark.asyncio
    async def test_search_trips_functionality(
        self, trip_service, mock_database_service
    ):
        """Test trip search functionality."""
        user_id = "user123"
        search_params = {"query": "Tokyo", "status": "planning", "limit": 10}
    ...
    @pytest.mark.asyncio
    async def test_trip_permission_validation(
        self, trip_service, mock_database_service
    ):
        """Test trip permission validation logic."""
        trip_id = "trip_123"
        owner_id = "owner123"
        # other_user_id = "other456"  # Not used in this test
