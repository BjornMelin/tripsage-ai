...
class MockActivityService:
    ...
    async def get_activity_details(self, activity_id: str):
        ...
        try:
            place_details = await self.google_maps_service.get_place_details(
                place_id, fields=["name", "formatted_address", "geometry", "rating", "price_level", "types", "opening_hours", "reviews", "website", "formatted_phone_number"]
            )
            ...
            if isinstance(e, MockGoogleMapsServiceError):
                ...
                raise MockActivityServiceError(f"Failed to get activity details: {e}", e)
    
    async def _convert_place_to_activity(self, place: Dict[str, Any], request: ActivitySearchRequest) -> Optional[ActivityResponse]:
        ...
        try:
            ...
            return ActivityResponse(
                ...
                date=request.start_date.isoformat() if request.start_date else datetime.now().date().isoformat(),
                duration=duration,
    ...
    async def _convert_detailed_place_to_activity(self, place: Dict[str, Any], activity_id: str) -> ActivityResponse:
        ...
        availability = "Open now" if opening_hours.get("open_now") else "Currently closed" if "open_now" in opening_hours else "Contact venue"
    ...
    def _apply_filters(self, activities: List[ActivityResponse], request: ActivitySearchRequest) -> List[ActivityResponse]:
...
class TestActivityService:
    ...
    async def test_search_activities_geocoding_error(self, activity_service, sample_activity_request):
        ...
        activity_service.google_maps_service.geocode.side_effect = MockGoogleMapsServiceError("Geocoding failed")
...
class TestAsyncBehavior:
    ...
    def activity_service(self):
        ...
        mock_gms = AsyncMock()
        mock_gms.geocode.return_value = [{"geometry": {"location": {"lat": 40.7128, "lng": -74.0060}}}]
        mock_gms.search_places.return_value = {"results": []}
        mock_gms.get_place_details.return_value = {"result": {"name": "Test", "types": ["establishment"]}}
        
        return MockActivityService(
            google_maps_service=mock_gms,
