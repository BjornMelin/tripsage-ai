...
class TestDatabaseConfig:
    ...
    def test_database_config_defaults(self):
        ...
        env_vars_to_clear = [
            ...
            "SUPABASE_TIMEOUT", "SUPABASE_AUTO_REFRESH_TOKEN", "SUPABASE_PERSIST_SESSION",
            "SUPABASE_PGVECTOR_ENABLED", "SUPABASE_VECTOR_DIMENSIONS"
        ]
    ...
    def test_database_config_environment_variables(self):
        ...
        with patch.dict(os.environ, env_vars):
            ...
            assert config.service_role_key.get_secret_value() == "custom-service-role-key"
...
class TestCoreAppSettings:
    ...
    def test_core_app_settings_validate_critical_settings_production_localhost_services(self):
        ...
        assert any("DragonflyDB is using localhost in production" in error for error in errors)
        assert any("Crawl4AI is using localhost in production" in error for error in errors)

    def test_core_app_settings_validate_critical_settings_production_default_secrets(self):
        ...
        assert any("API key master secret must be changed in production" in error for error in errors)
        assert any("Supabase JWT secret must be changed in production" in error for error in errors)

    def test_core_app_settings_validate_critical_settings_production_fallback_secret(self):
...
class TestConfigurationLoading:
    ...
    def test_init_settings_validation_failure(self):
        ...
        with patch('tripsage_core.config.base_app_settings.get_settings') as mock_get_settings:
            mock_settings = Mock()
    ...
    def test_configuration_inheritance_and_overrides(self):
        ...
        custom_dragonfly = DragonflyConfig(
            url="redis://override:6379/1",
            ttl_short=600
        )
        
        settings = CoreAppSettings(
            database=custom_database,
            dragonfly=custom_dragonfly
        )
...
class TestProductionReadinessValidation:
    ...
    def test_staging_environment_validation(self):
        ...
        assert len(errors) == 0 or all("production" not in error.lower() for error in errors)
