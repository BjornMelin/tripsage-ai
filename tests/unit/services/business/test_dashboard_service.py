...
class TestDashboardService:
    ...
    async def test_get_real_time_metrics_with_cache(
        self, 
        dashboard_service
    ):
    ...
    async def test_get_user_activity_data(
        self, 
        dashboard_service, 
        sample_usage_logs
    ):
    ...
    async def test_get_rate_limit_status_from_cache(
        self, 
        dashboard_service
    ):
    ...
    async def test_get_rate_limit_status_without_cache(
        self, 
        dashboard_service
    ):
    ...
    async def test_alert_priority_scoring(self, dashboard_service):
        ...
        critical_alert = await dashboard_service.create_alert(
            AlertType.SYSTEM_OVERLOAD, AlertSeverity.CRITICAL, "Critical Issue", "System overload"
        )
        
        high_alert = await dashboard_service.create_alert(
            AlertType.HIGH_ERROR_RATE, AlertSeverity.HIGH, "High Error Rate", "Error rate high"
        )
        
        medium_alert = await dashboard_service.create_alert(
            AlertType.PERFORMANCE_DEGRADATION, AlertSeverity.MEDIUM, "Performance Issue", "Slow response"
        )
        ...
        # Note: priority score is computed property, so we need to check the actual alert
    ...
    async def test_get_usage_trends_by_hour(
        self, 
        dashboard_service, 
        sample_usage_logs
    ):
        ...
        async def mock_query_logs(start_time, end_time):
            ...
            return [
                log for log in sample_usage_logs 
    ...
    async def test_overall_health_score_calculation(self, sample_health_checks):
        ...
        # With 95% success rate and mixed service health, score should be good but not perfect
