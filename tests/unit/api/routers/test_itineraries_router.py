...
class TestItinerariesRouter:
    ...
    def sample_item_create_request(self):
        ...
        from tripsage.api.schemas.itineraries import Location, TimeSlot, ItineraryItemType
    ...
    async def test_create_itinerary_success(self, mock_principal, mock_itinerary_service, sample_create_request):
        ...
        mock_itinerary_service.create_itinerary.assert_called_once_with("user123", sample_create_request)
        assert result == expected_itinerary
        assert result["title"] == "Tokyo Adventure"

    async def test_create_itinerary_invalid_data(self, mock_principal, mock_itinerary_service, sample_create_request):
        """Test itinerary creation with invalid data."""
        mock_itinerary_service.create_itinerary.side_effect = ValueError("Invalid date range")

        with pytest.raises(HTTPException) as exc_info:
            await create_itinerary(sample_create_request, mock_principal, mock_itinerary_service)

        assert exc_info.value.status_code == 400
        assert "Invalid date range" in str(exc_info.value.detail)

    async def test_list_itineraries_success(self, mock_principal, mock_itinerary_service):
    ...
    async def test_search_itineraries_success(self, mock_principal, mock_itinerary_service, sample_search_request):
        ...
        result = await search_itineraries(sample_search_request, mock_principal, mock_itinerary_service)

        mock_itinerary_service.search_itineraries.assert_called_once_with("user123", sample_search_request)
        assert result == expected_response
        assert "itineraries" in result
        assert "total_count" in result

    async def test_search_itineraries_no_results(self, mock_principal, mock_itinerary_service):
        ...
        result = await search_itineraries(search_request, mock_principal, mock_itinerary_service)
    ...
    async def test_get_itinerary_success(self, mock_principal, mock_itinerary_service):
        ...
        result = await get_itinerary(itinerary_id, mock_principal, mock_itinerary_service)

        mock_itinerary_service.get_itinerary.assert_called_once_with("user123", itinerary_id)
        assert result == expected_itinerary
        assert result["id"] == itinerary_id

    async def test_get_itinerary_not_found(self, mock_principal, mock_itinerary_service):
        """Test itinerary retrieval for non-existent itinerary."""
        itinerary_id = "nonexistent"
        mock_itinerary_service.get_itinerary.side_effect = CoreResourceNotFoundError("Itinerary not found")
    ...
    async def test_update_itinerary_success(self, mock_principal, mock_itinerary_service, sample_update_request):
    ...
    async def test_update_itinerary_not_found(self, mock_principal, mock_itinerary_service, sample_update_request):
        """Test itinerary update for non-existent itinerary."""
        itinerary_id = "nonexistent"
        mock_itinerary_service.update_itinerary.side_effect = CoreResourceNotFoundError("Itinerary not found")
    ...
    async def test_update_itinerary_invalid_data(self, mock_principal, mock_itinerary_service, sample_update_request):
        """Test itinerary update with invalid data."""
        itinerary_id = "itinerary123"
        mock_itinerary_service.update_itinerary.side_effect = ValueError("Invalid update data")
    ...
    async def test_delete_itinerary_success(self, mock_principal, mock_itinerary_service):
        ...
        result = await delete_itinerary(itinerary_id, mock_principal, mock_itinerary_service)

        mock_itinerary_service.delete_itinerary.assert_called_once_with("user123", itinerary_id)
        assert result is None

    async def test_delete_itinerary_not_found(self, mock_principal, mock_itinerary_service):
        """Test itinerary deletion for non-existent itinerary."""
        itinerary_id = "nonexistent"
        mock_itinerary_service.delete_itinerary.side_effect = CoreResourceNotFoundError("Itinerary not found")
    ...
    async def test_add_item_to_itinerary_success(self, mock_principal, mock_itinerary_service, sample_item_create_request):
    ...
    async def test_add_item_to_itinerary_not_found(self, mock_principal, mock_itinerary_service, sample_item_create_request):
        """Test adding item to non-existent itinerary."""
        itinerary_id = "nonexistent"
        mock_itinerary_service.add_item_to_itinerary.side_effect = CoreResourceNotFoundError("Itinerary not found")
    ...
    async def test_add_item_invalid_data(self, mock_principal, mock_itinerary_service, sample_item_create_request):
        """Test adding item with invalid data."""
        itinerary_id = "itinerary123"
        mock_itinerary_service.add_item_to_itinerary.side_effect = ValueError("Invalid time range")
    ...
    async def test_get_itinerary_item_success(self, mock_principal, mock_itinerary_service):
        ...
        result = await get_itinerary_item(itinerary_id, item_id, mock_principal, mock_itinerary_service)

        mock_itinerary_service.get_item.assert_called_once_with("user123", itinerary_id, item_id)
        assert result == expected_item
        assert result["id"] == item_id

    async def test_get_itinerary_item_not_found(self, mock_principal, mock_itinerary_service):
        """Test itinerary item retrieval for non-existent item."""
        itinerary_id = "itinerary123"
        item_id = "nonexistent"
        mock_itinerary_service.get_item.side_effect = CoreResourceNotFoundError("Item not found")

        with pytest.raises(HTTPException) as exc_info:
            await get_itinerary_item(itinerary_id, item_id, mock_principal, mock_itinerary_service)

        assert exc_info.value.status_code == 404
        assert "Item not found" in str(exc_info.value.detail)

    async def test_update_itinerary_item_success(self, mock_principal, mock_itinerary_service, sample_item_update_request):
    ...
    async def test_update_itinerary_item_not_found(self, mock_principal, mock_itinerary_service, sample_item_update_request):
        """Test itinerary item update for non-existent item."""
        itinerary_id = "itinerary123"
        item_id = "nonexistent"
        mock_itinerary_service.update_item.side_effect = CoreResourceNotFoundError("Item not found")
    ...
    async def test_update_itinerary_item_invalid_data(self, mock_principal, mock_itinerary_service, sample_item_update_request):
    ...
    async def test_delete_itinerary_item_success(self, mock_principal, mock_itinerary_service):
        ...
        # Should not raise an exception and return None
        result = await delete_itinerary_item(itinerary_id, item_id, mock_principal, mock_itinerary_service)

        mock_itinerary_service.delete_item.assert_called_once_with("user123", itinerary_id, item_id)
        assert result is None

    async def test_delete_itinerary_item_not_found(self, mock_principal, mock_itinerary_service):
        """Test itinerary item deletion for non-existent item."""
        itinerary_id = "itinerary123"
        item_id = "nonexistent"
        mock_itinerary_service.delete_item.side_effect = CoreResourceNotFoundError("Item not found")

        with pytest.raises(HTTPException) as exc_info:
            await delete_itinerary_item(itinerary_id, item_id, mock_principal, mock_itinerary_service)

        assert exc_info.value.status_code == 404
        assert "Item not found" in str(exc_info.value.detail)

    async def test_check_itinerary_conflicts_success(self, mock_principal, mock_itinerary_service):
        ...
        result = await check_itinerary_conflicts(itinerary_id, mock_principal, mock_itinerary_service)

        mock_itinerary_service.check_conflicts.assert_called_once_with("user123", itinerary_id)
        assert result == expected_conflicts
        assert "has_conflicts" in result
        assert "conflicts" in result

    async def test_check_itinerary_conflicts_with_conflicts(self, mock_principal, mock_itinerary_service):
    ...
    async def test_check_itinerary_conflicts_not_found(self, mock_principal, mock_itinerary_service):
        """Test conflict checking for non-existent itinerary."""
        itinerary_id = "nonexistent"
        mock_itinerary_service.check_conflicts.side_effect = CoreResourceNotFoundError("Itinerary not found")

        with pytest.raises(HTTPException) as exc_info:
            await check_itinerary_conflicts(itinerary_id, mock_principal, mock_itinerary_service)

        assert exc_info.value.status_code == 404
        assert "Itinerary not found" in str(exc_info.value.detail)

    async def test_optimize_itinerary_success(self, mock_principal, mock_itinerary_service, sample_optimize_request):
        ...
        result = await optimize_itinerary(sample_optimize_request, mock_principal, mock_itinerary_service)

        mock_itinerary_service.optimize_itinerary.assert_called_once_with("user123", sample_optimize_request)
        assert result == expected_optimization
        assert "optimized_itinerary" in result
        assert "improvements" in result

    async def test_optimize_itinerary_not_found(self, mock_principal, mock_itinerary_service, sample_optimize_request):
        """Test optimization for non-existent itinerary."""
        mock_itinerary_service.optimize_itinerary.side_effect = CoreResourceNotFoundError("Itinerary not found")

        with pytest.raises(HTTPException) as exc_info:
            await optimize_itinerary(sample_optimize_request, mock_principal, mock_itinerary_service)

        assert exc_info.value.status_code == 404
        assert "Itinerary not found" in str(exc_info.value.detail)

    async def test_optimize_itinerary_invalid_request(self, mock_principal, mock_itinerary_service, sample_optimize_request):
        """Test optimization with invalid request."""
        mock_itinerary_service.optimize_itinerary.side_effect = ValueError("Invalid optimization settings")
