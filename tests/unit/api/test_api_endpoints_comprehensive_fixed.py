...
with patch.dict('sys.modules', {
    ...
    from tripsage.api.models.requests.auth import RegisterUserRequest, LoginRequest
...
class TestAuthEndpoints:
    ...
    async def test_login_user_success(self, mock_auth_service):
        ...
        user = await mock_auth_service.authenticate_user(login_data["username"], login_data["password"])
    ...
    def test_trip_create_model_date_validation(self):
        ...
        with pytest.raises(Exception):  # Should raise validation error
            CreateTripRequest(
                ...
                end_date=end_date,
                destinations=[
                    TripDestination(name="Test Destination")
                ]
            )
