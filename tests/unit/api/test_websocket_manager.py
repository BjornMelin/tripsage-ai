...
class TestWebSocketManager:
    ...
    async def test_remove_connection(self, websocket_manager, mock_websocket):
        ...
        assert connection_id not in websocket_manager.session_connections.get(session_id, [])

    @pytest.mark.asyncio
    async def test_authenticate_connection_success(self, websocket_manager, mock_websocket, mock_jwt_token):
        ...
        with patch("tripsage.api.services.websocket_manager.jwt.decode") as mock_decode:
            ...
            result = await websocket_manager.authenticate_connection(mock_websocket, auth_request)
    ...
    @pytest.mark.asyncio
    async def test_authenticate_connection_expired_token(self, websocket_manager, mock_websocket):
    ...
    @pytest.mark.asyncio
    async def test_health_check_removes_unhealthy_connections(self, websocket_manager, mock_websocket):
