...
class TestUnifiedSearchEndpoint:
    ...
    @pytest.fixture
    def sample_unified_response(self, sample_search_result, sample_search_metadata, sample_search_facet):
    ...
    async def test_unified_search_success(self, async_client: AsyncClient, sample_search_request, sample_unified_response):
    ...
    async def test_unified_search_empty_results(self, async_client: AsyncClient, sample_search_request):
    ...
    async def test_unified_search_with_filters(self, async_client: AsyncClient):
        ...
        with patch('tripsage.api.routers.search.get_unified_search_service') as mock_get_service:
            ...
            response = await async_client.post(
                "/search/unified",
                json=request_with_filters.model_dump()
            )
    ...
    async def test_unified_search_service_error(self, async_client: AsyncClient, sample_search_request):
        """Test unified search with service error."""
        with patch('tripsage.api.routers.search.get_unified_search_service') as mock_get_service:
            mock_service = AsyncMock()
            mock_service.unified_search.side_effect = UnifiedSearchServiceError("External API timeout")
            mock_get_service.return_value = mock_service
    ...
    async def test_unified_search_unexpected_error(self, async_client: AsyncClient, sample_search_request):
        """Test unified search with unexpected error."""
        with patch('tripsage.api.routers.search.get_unified_search_service') as mock_get_service:
            mock_service = AsyncMock()
            mock_service.unified_search.side_effect = Exception("Database connection lost")
            mock_get_service.return_value = mock_service
    ...
    async def test_unified_search_missing_required_fields(self, async_client: AsyncClient):
    ...
    async def test_unified_search_various_types(self, async_client: AsyncClient, sample_unified_response):
...
class TestSearchSuggestionsEndpoint:
    ...
    async def test_search_suggestions_invalid_limit(self, async_client: AsyncClient):
        ...
        test_cases = [
            ("?query=test&limit=0", status.HTTP_422_UNPROCESSABLE_ENTITY),  # Below minimum
            ("?query=test&limit=25", status.HTTP_422_UNPROCESSABLE_ENTITY),  # Above maximum
            ("?query=test&limit=-5", status.HTTP_422_UNPROCESSABLE_ENTITY),  # Negative
            ("?query=test&limit=abc", status.HTTP_422_UNPROCESSABLE_ENTITY),  # Non-numeric
        ]
    ...
    async def test_search_suggestions_various_queries(self, async_client: AsyncClient):
        ...
        with patch('tripsage.api.routers.search.get_unified_search_service') as mock_get_service:
            ...
            for query in test_queries:
                response = await async_client.get(f"/search/suggest?query={query}&limit=5")
    ...
    async def test_search_suggestions_service_error(self, async_client: AsyncClient):
        ...
        with patch('tripsage.api.routers.search.get_unified_search_service') as mock_get_service:
            mock_service = AsyncMock()
            mock_service.get_search_suggestions.side_effect = UnifiedSearchServiceError("Suggestion service down")
            mock_get_service.return_value = mock_service
    ...
    async def test_search_suggestions_unexpected_error(self, async_client: AsyncClient):
        ...
        with patch('tripsage.api.routers.search.get_unified_search_service') as mock_get_service:
            mock_service = AsyncMock()
            mock_service.get_search_suggestions.side_effect = Exception("Redis connection failed")
            mock_get_service.return_value = mock_service
            ...
            assert "An unexpected error occurred" in data["detail"]
            assert "Redis connection failed" not in data["detail"]  # Should not expose internal error
    ...
    async def test_concurrent_requests(self, async_client: AsyncClient):
        ...
        with patch('tripsage.api.routers.search.get_unified_search_service') as mock_get_service:
            ...
            mock_service.unified_search.return_value = UnifiedSearchResponse(
                results=[], facets=[], 
                metadata=SearchMetadata(
                    total_results=0, returned_results=0, search_time_ms=100,
                    search_id="concurrent", providers_queried=[]
                ),
                results_by_type={}, errors=None
            )
            mock_get_service.return_value = mock_service
    ...
    async def test_limit_boundary_validation(self, async_client: AsyncClient):
        ...
        with patch('tripsage.api.routers.search.get_unified_search_service') as mock_get_service:
            ...
            for limit, expected_status in test_cases:
                response = await async_client.get(f"/search/suggest?query=test&limit={limit}")
                assert response.status_code == expected_status
    ...
    async def test_special_characters_in_query(self, async_client: AsyncClient):
        ...
        with patch('tripsage.api.routers.search.get_unified_search_service') as mock_get_service:
            ...
            for query in special_queries:
                response = await async_client.get(f"/search/suggest", params={"query": query, "limit": 5})
                assert response.status_code == status.HTTP_200_OK
    ...
    async def test_large_request_payload(self, async_client: AsyncClient):
        ...
        with patch('tripsage.api.routers.search.get_unified_search_service') as mock_get_service:
            ...
            mock_service.unified_search.return_value = UnifiedSearchResponse(
                ...
                metadata=SearchMetadata(
                    total_results=0, returned_results=0, search_time_ms=100,
                    search_id="large", providers_queried=[]
                ),
                results_by_type={}, errors=None
            )
            mock_get_service.return_value = mock_service
...
class TestLogging:
    ...
    async def test_request_logging(self, async_client: AsyncClient):
        ...
        with patch('tripsage.api.routers.search.logger') as mock_logger, \
             patch('tripsage.api.routers.search.get_unified_search_service') as mock_get_service:
            ...
            mock_service.unified_search.return_value = UnifiedSearchResponse(
                ...
                metadata=SearchMetadata(
                    total_results=0, returned_results=0, search_time_ms=100,
                    search_id="log_test", providers_queried=[]
                ),
                results_by_type={}, errors=None
            )
            mock_get_service.return_value = mock_service
    ...
    async def test_error_logging(self, async_client: AsyncClient):
        ...
        with patch('tripsage.api.routers.search.logger') as mock_logger, \
            ...
            mock_service = AsyncMock()
            mock_service.unified_search.side_effect = UnifiedSearchServiceError("Test error")
            mock_get_service.return_value = mock_service
