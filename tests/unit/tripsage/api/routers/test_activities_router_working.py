"""
Working tests for Activities API router endpoints (BJO-120 implementation).
...
from tripsage.api.schemas.responses.activities import ActivityCoordinates, ActivityResponse
...
def authenticated_client(app: FastAPI) -> TestClient:
    """Create authenticated test client with properly mocked services."""
...
@pytest.fixture
def sample_activity_response() -> ActivityResponse:
...
@pytest.fixture
def sample_saved_activity() -> Dict[str, any]:
    """Create sample saved activity data."""
    return {
        "id": str(uuid.uuid4()),
        "user_id": "user123", 
...
class TestSaveActivityEndpoint:
    ...
    def test_save_activity_success(
        ...
        sample_activity_response: ActivityResponse,
        sample_saved_activity: Dict[str, any],
    ):
        ...
        with patch("tripsage.api.routers.activities.get_activity_service") as mock_get_service:
            ...
            mock_service.save_activity.return_value = sample_saved_activity
            mock_service.get_activity_details.return_value = sample_activity_response
...
class TestGetSavedActivitiesEndpoint:
    ...
    def test_get_saved_activities_success(
        ...
        saved_activities = [sample_saved_activity]
...
class TestRouterIntegration:
    ...
    def test_end_to_end_activity_workflow(
        ...
        workflow_saved_data = {**sample_saved_activity, "activity_id": activity_id}
        workflow_activity_details = ActivityResponse(
            **sample_activity_response.model_dump(), id=activity_id
        )
        
        saved_activities_response = [{
            **workflow_saved_data,
            "activity_data": workflow_activity_details.model_dump()
        }]
