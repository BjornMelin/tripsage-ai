...
class TestSimpleTools:
    ...
    async def test_web_search_tool(self, mock_mcp_manager):
        ...
        mock_result = {
            "results": [
                {"title": "Best time to visit Paris", "url": "example.com"}
            ]
        }
        mock_mcp_manager.invoke = AsyncMock(return_value=mock_result)
    ...
    async def test_memory_tools(self, mock_mcp_manager):
        ...
        mock_mcp_manager.invoke = AsyncMock(return_value={
            "memories": [
    ...
    def test_tool_descriptions(self):
        ...
        for tool in ALL_TOOLS:
            ...
            if "memory" in tool.name:
                ...
                assert ("travel" in tool.description.lower() or 
                       "location" in tool.description.lower() or
                       "weather" in tool.description.lower() or
                       "search" in tool.description.lower() or
                       "accommodation" in tool.description.lower() or
                       "flight" in tool.description.lower())