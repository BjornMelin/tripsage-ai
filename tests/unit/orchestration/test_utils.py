...
class MockChatOpenAI:
    ...
    async def ainvoke(self, messages: List[Dict[str, str]], **kwargs) -> MockLLMResponse:
        """Mock async invoke."""
        ...
        if messages:
            ...
            if "extract" in content.lower() and "parameters" in content.lower():
                ...
                if "accommodation" in content.lower():
                    return MockLLMResponse('{"location": "Paris", "check_in": "2024-06-15", "check_out": "2024-06-20"}')
                elif "flight" in content.lower():
                    return MockLLMResponse('{"origin": "NYC", "destination": "LAX", "departure_date": "2024-06-15"}')
                elif "budget" in content.lower():
                    # Check if it's asking about a specific budget amount
                    if "$2000" in content:
                        return MockLLMResponse('{"operation": "optimize", "total_budget": 2000, "trip_length": 7}')
                    else:
                        return MockLLMResponse('{"operation": "optimize", "total_budget": 2000, "trip_length": 7}')
                elif "destination" in content.lower():
                    return MockLLMResponse('{"destination": "Tokyo", "research_type": "overview"}')
                elif "itinerary" in content.lower():
                    return MockLLMResponse('{"operation": "create", "destination": "Rome", "duration": 5}')
                else:
            ...
            if "classify" in content.lower() and "intent" in content.lower():
                if "flight" in content.lower():
                    return MockLLMResponse(
                        '{"agent": "flight_agent", "confidence": 0.9, "reasoning": "User mentioned flights"}'
                    )
                elif "hotel" in content.lower() or "accommodation" in content.lower():
                    return MockLLMResponse(
                        '{"agent": "accommodation_agent", "confidence": 0.9, "reasoning": "User mentioned hotels"}'
                    )
                elif "budget" in content.lower():
                    return MockLLMResponse(
            ...
            if "provide a helpful response" in content.lower() or "general" in content.lower():
                if "accommodation" in content.lower():
                    return MockLLMResponse(
            ...
            if "generate" in content.lower() and "response" in content.lower():
                if "accommodation" in content.lower():
                    ...
                    return MockLLMResponse("I found several flight options from NYC to LAX.")
                elif "budget" in content.lower():
                    return MockLLMResponse("I've optimized your $2000 budget for a 7-day trip.")
                elif "destination" in content.lower():
                    return MockLLMResponse("Tokyo is an amazing destination with rich culture!")
                elif "itinerary" in content.lower():
...
AGENT_TEST_RESPONSES = {
    "flight_agent": {
        "parameter_extraction": ('{"origin": "NYC", "destination": "LAX", "departure_date": "2024-06-15"}'),
        "search_result": {"flights": [{"id": "FL123", "price": 299.99}]},
        "response": "I found 1 flight from NYC to LAX for $299.99.",
    },
    "accommodation_agent": {
        "parameter_extraction": ('{"location": "Paris", "check_in": "2024-06-15", "check_out": "2024-06-20"}'),
        "search_result": {"accommodations": [{"id": "H456", "name": "Hotel Paris", "price": 150.0}]},
        "response": "I found Hotel Paris in Paris for $150 per night.",
    },
    "budget_agent": {
        "parameter_extraction": ('{"operation": "optimize", "total_budget": 2000, "trip_length": 7}'),
        "optimization_result": {
            "allocations": {
                "flights": 600,
                "accommodation": 700,
        ...
        "response": ("I've optimized your $2000 budget: Flights $600, Accommodation $700, Food $400, Activities $300."),
    },
    "destination_research_agent": {
        "parameter_extraction": ('{"destination": "Tokyo", "research_type": "overview"}'),
        "research_result": {
            "overview": "Tokyo is Japan's capital...",
            "attractions": ["Senso-ji Temple", "Tokyo Tower"],
        },
        "response": ("Tokyo is an amazing destination! Top attractions include Senso-ji Temple and Tokyo Tower."),
    },
    "itinerary_agent": {
        "parameter_extraction": ('{"operation": "create", "destination": "Rome", "duration": 5}'),
        "itinerary_result": {"daily_schedule": [{"day": 1, "activities": ["Colosseum", "Roman Forum"]}]},
        "response": ("I've created a 5-day itinerary for Rome. Day 1: Visit Colosseum and Roman Forum."),
    },
}
