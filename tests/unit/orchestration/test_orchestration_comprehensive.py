...
def optimized_orchestrator(
    ...
    """
    with patch("tripsage.orchestration.graph.get_memory_bridge", return_value=mock_memory_bridge):
        with patch("tripsage.orchestration.graph.get_handoff_coordinator", return_value=mock_handoff_coordinator):
...
class TestBaseAgentNodeComprehensive:
    ...
    async def test_concurrent_node_execution(self, comprehensive_service_registry):
        ...
        """
        nodes = [
            MockAgentNode(comprehensive_service_registry, f"node_{i}", process_delay=0.01)
            for i in range(3)
        ]
        
        states = [
            create_initial_state(f"user-{i}", f"Message {i}")
            for i in range(3)
        ]
        
        # Execute concurrently
        tasks = [node(state) for node, state in zip(nodes, states)]
...
class TestTripSageOrchestratorOptimized:
    ...
    def test_orchestrator_initialization_patterns(self, comprehensive_service_registry):
        ...
        with patch("tripsage.orchestration.graph.get_memory_bridge"):
            with patch("tripsage.orchestration.graph.get_handoff_coordinator"):
                with patch("tripsage.orchestration.graph.get_default_config"):
                    # Test with minimal configuration - provide service registry to avoid initialization errors
    ...
    def test_routing_decision_matrix(self, optimized_orchestrator):
        ...
        test_cases = [
            ...
            ({"current_agent": "destination_research_agent"}, "destination_research_agent"),
            ({"current_agent": "general_agent"}, "general_agent"),
            ({"current_agent": None, "error_info": {"error_count": 0}}, "general_agent"),
            ({"current_agent": None, "error_info": {"error_count": 3}}, "error_recovery"),
            ({"current_agent": "invalid_agent", "error_info": {"error_count": 1}}, "general_agent"),
