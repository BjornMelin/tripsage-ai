...
class TestDuffelHTTPClient:
    ...
    def mock_settings(self):
        ...
        with patch("tripsage.services.duffel_http_client.settings") as mock_settings:
            mock_settings.flights_mcp.duffel_api_key.get_secret_value.return_value = (
                "test-api-key"
            )
            yield mock_settings
    ...
    async def test_search_flights_success(
        self, client, mock_flight_search_params, mock_duffel_response
    ):
    ...
    @pytest.mark.asyncio
    async def test_search_flights_request_format(
        self, client, mock_flight_search_params
    ):
    ...
    async def test_get_aircraft_success(self, client):
        ...
        aircraft_data = {
            "data": {"id": "arc_12345", "name": "Boeing 737-800", "iata_code": "738"}
        }
    ...
    async def test_list_aircraft_success(self, client):
        ...
        with patch.object(client, "_make_request") as mock_request:
            ...
            mock_request.assert_called_once_with(
                "GET", "/air/aircraft", params={"limit": 10}
            )
...
class TestDuffelAPIError:
    ...
    def test_duffel_api_error_with_details(self):
        ...
        error = DuffelAPIError(
            "Test error", status_code=400, response_data=response_data
        )
