...
class PerformanceBenchmark:
    ...
    async def benchmark_operation(self, name: str, operation, *args, **kwargs) -> Dict[str, float]:
        ...
        if not times:
            return {"mean": float('inf'), "median": float('inf'), "min": float('inf'), "max": float('inf')}
...
class TestSupabaseMigrationPerformance:
    ...
    async def test_supabase_connection_performance(self, supabase_service, benchmark):
        ...
        connection_stats = await benchmark.benchmark_operation(
            "supabase_connect",
            supabase_service.ensure_connected
        )
...
async def run_comprehensive_benchmark():
    ...
    print("Note: Performance improvements are measured against the previous MCP implementation.")
