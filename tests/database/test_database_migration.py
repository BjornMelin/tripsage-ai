...
with patch('tripsage.config.app_settings.settings'):
    from tripsage.tools import supabase_tools, memory_tools
...
class TestUserOperations(TestDatabaseMigrationCompleteness):
    ...
    async def test_update_user_preferences(self, mock_mcp_manager, mock_supabase_response):
        ...
        with patch('tripsage.tools.supabase_tools.get_mcp_manager', return_value=mock_mcp_manager):
...
class TestNeo4jOperations(TestDatabaseMigrationCompleteness):
    ...
    async def test_find_destinations_by_country(self, mock_mcp_manager, mock_memory_response):
        ...
        with patch('tripsage.tools.memory_tools.get_mcp_manager', return_value=mock_mcp_manager):
    ...
    async def test_create_trip_entities(self, mock_mcp_manager, mock_memory_response):
        ...
        mock_mcp_manager.invoke.return_value = mock_memory_response({"status": "success"})
...
class TestMigrationRunnners(TestDatabaseMigrationCompleteness):
    ...
    async def test_sql_migration_runner(self, mock_mcp_manager, mock_supabase_response):
        ...
        mock_mcp_manager.invoke.return_value = mock_supabase_response({"status": "success"})
        
        with patch('tripsage.db.migrations.runner.MCPManager.get_instance', return_value=mock_mcp_manager):
...
class TestInitialization(TestDatabaseMigrationCompleteness):
    ...
    async def test_initialize_databases(self, mock_mcp_manager):
        ...
        mock_mcp_manager.invoke.return_value = Mock(error=None, result={"version": "14.0"})
        
        with patch('tripsage.db.initialize.MCPManager.get_instance', return_value=mock_mcp_manager):
