...
class TestJWTBypassAttempts:
    ...
    def test_client_with_auth(self, test_app, valid_jwt_secret) -> TestClient:
        ...
        with patch("tripsage.api.core.config.get_settings") as mock_settings:
            mock_settings.return_value.database_jwt_secret.get_secret_value.return_value = valid_jwt_secret
            
            middleware = AuthenticationMiddleware(test_app)
            test_app.add_middleware(
                type(middleware).__bases__[0],
...
class TestAPIKeyBypassAttempts:
    ...
    def test_api_key_format_bypass_attempts(self, test_client_with_api_key_auth):
        ...
        malicious_api_keys = [
            ...
            "%73%6b%5f%6f%70%65%6e%61%69%5f%31%32%33%5f%73%65%63%72%65%74",  # URL encoded
    ...
    def test_api_key_header_injection_attacks(self, test_client_with_api_key_auth):
        ...
        injection_headers = [
            ...
            {"X-API-Key": "sk_openai_123_secret\r\n\r\n<script>alert('xss')</script>"},
            {"X-API-Key": "sk_openai_123_secret", "X-API-Key-Backup": "sk_admin_999_backdoor"},
    ...
    def test_api_key_replay_attack_simulation(self, test_client_with_api_key_auth, mock_key_service):
        ...
        with patch("tripsage_core.services.business.key_management_service.get_key_management_service", return_value=mock_key_service):
            ...
            time.sleep(0.1)
            response2 = test_client_with_api_key_auth.get(
                "/api-protected", 
                headers={"X-API-Key": valid_api_key}
            )
            ...
            if response1.status_code == 200:
                # If first request succeeded, replay protection depends on implementation
                pass
...
class TestAuthenticationHeaderManipulation:
    ...
    def test_authorization_header_injection(self, test_client_full_auth):
        ...
        for malicious_auth in injection_attempts:
            response = test_client_full_auth.get(
                "/protected",
                headers={"Authorization": malicious_auth}
            )
    ...
    def test_header_size_limits(self, test_client_full_auth):
        ...
        response = test_client_full_auth.get(
            "/protected",
            headers={large_header_name: "value"}
        )
        assert response.status_code in [400, 413, 431]  # Bad request or header too large
...
class TestPrivilegeEscalationAttempts:
    ...
    def test_role_manipulation_in_jwt(self, test_app_with_roles):
        ...
        with patch("tripsage.api.core.config.get_settings") as mock_settings:
            mock_settings.return_value.database_jwt_secret.get_secret_value.return_value = "test_secret"
...
class TestInformationDisclosureViaAuthentication:
    ...
    def test_sensitive_data_in_error_responses(self, test_client_with_auth):
        ...
        for token in malicious_tokens:
            ...
            sensitive_terms = [
                "secret", "key", "password", "database", "config",
