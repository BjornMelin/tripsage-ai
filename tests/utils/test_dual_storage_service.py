...
from src.utils.trip_storage_service import TripPrimaryModel, TripGraphModel, TripStorageService
...
class TestDualStorageService:
    ...
    class MockDualStorageService(DualStorageService[MockPrimaryModel, MockGraphModel]):
        ...
        async def _create_graph_relations(
            self, data: Dict[str, Any], entity_id: str, created_entities: List[Dict[str, Any]]
        ) -> List[Dict[str, Any]]:
            relations = [{
    ...
    async def test_delete(self, mock_service, mock_primary_client, mock_graph_client):
        ...
        mock_graph_client.delete_entities.assert_called_once_with(["Entity:test-id-123"])
...
class TestTripStorageService:
    ...
    @pytest.mark.asyncio
    async def test_trip_service_create(self, mock_db_client, mock_memory_client, sample_trip_data):
    ...
    async def test_trip_service_delete(self, mock_db_client, mock_memory_client):
        ...
        with patch('src.utils.trip_storage_service.db_client', mock_db_client), \
            ...
            mock_memory_client.delete_entities.assert_called_once_with(["Trip:trip-123"])
    ...
    @pytest.mark.asyncio
    async def test_trip_service_with_error_handling(self, mock_db_client, mock_memory_client):
    ...
    @pytest.mark.asyncio
    async def test_retrieve_with_nonexistent_trip(self, mock_db_client, mock_memory_client):
    ...
    @pytest.mark.asyncio
    async def test_update_with_memory_client_error(self, mock_db_client, mock_memory_client):
