...
class TestChatAgentUnit:
    ...
    def test_intent_detection_algorithm(self):
        ...
        for message, expected_intent, should_have_confidence in test_cases:
            ...
            if should_have_confidence:
                ...
                assert result["all_scores"][expected_intent] > 0, (
                    f"No score for: {message}"
                )
            else:
    ...
    def test_rate_limiting_algorithm(self):
        ...
        def check_rate_limit_logic(
            call_history: list, max_calls: int, window_seconds: int, current_time: float
        ) -> bool:
            """Rate limiting logic from ChatAgent."""
            # Remove calls older than window
            recent_calls = [
                call_time
                for call_time in call_history
        ...
        for i in range(5):
            assert check_rate_limit_logic(
                call_history, max_calls, window_seconds, current_time + i
            )
            call_history = log_call_logic(call_history, current_time + i)

        # Should deny 6th call
        assert not check_rate_limit_logic(
            call_history, max_calls, window_seconds, current_time + 5
        )

        # Should allow calls after time window
        future_time = current_time + 61
        assert check_rate_limit_logic(
            call_history, max_calls, window_seconds, future_time
        )
    ...
    async def test_tool_execution_algorithm(self):
        ...
        async def execute_tool_logic(
            tool_name: str, parameters: dict, mock_manager
        ) -> dict:
        ...
        result = await execute_tool_logic(
            "weather_tool", {"location": "Paris"}, mock_manager
        )
    ...
    def test_agent_routing_algorithm(self):
            ...
            if confidence > 0.7:
                if primary_intent == "flight":
                    return {
                        ...
                        "reason": "High confidence flight intent",
                    }
                elif primary_intent == "accommodation":
                    return {
                        "route_to": "accommodation_agent",
                        "reason": "High confidence accommodation intent",
                    }
                elif primary_intent == "budget":
                    return {
                        "route_to": "budget_agent",
                        "reason": "High confidence budget intent",
                    }
                elif primary_intent == "destination":
                    return {
                        "route_to": "destination_agent",
                        "reason": "High confidence destination intent",
                    }
                elif primary_intent == "itinerary":
                    return {
                        "route_to": "itinerary_agent",
                        "reason": "High confidence itinerary intent",
                    }

            # Default to general travel agent or direct handling
            return {
                "route_to": "travel_agent",
                "reason": f"Low confidence or general intent ({primary_intent})",
            }
        ...
        test_cases = [
            ...
            (
                {"primary_intent": "accommodation", "confidence": 0.9},
                "accommodation_agent",
            ),
            ({"primary_intent": "budget", "confidence": 0.75}, "budget_agent"),
            (
                {"primary_intent": "flight", "confidence": 0.6},
                "travel_agent",
            ),  # Low confidence
            ({"primary_intent": "general", "confidence": 0.5}, "travel_agent"),
            (
                {"primary_intent": "weather", "confidence": 0.8},
                "travel_agent",
            ),  # Not routable
        ]
        ...
        for intent, expected_route in test_cases:
            ...
            assert routing["route_to"] == expected_route, (
                f"Failed routing for: {intent}"
            )
