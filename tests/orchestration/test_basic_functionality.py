...
def mock_config():
    ...
    with patch('tripsage.orchestration.config.get_settings') as mock_get_settings:
        mock_settings = Mock()
...
def sample_state():
    ...
    return TravelPlanningState(
        ...
        messages=[
            {
                "role": "user",
                "content": "I need help finding flights from NYC to Paris",
                "timestamp": "2024-01-01T10:00:00Z"
            }
        ],
        current_agent="router",
        agent_history=["router"],
        user_preferences={"budget": "moderate", "class": "economy"},
...
class TestTripSageOrchestrator:
    ...
    async def test_flight_agent_processing(self, mock_config, sample_state):
        ...
        with patch('tripsage.mcp_abstraction.manager.MCPManager') as mock_manager:
            ...
            mock_manager_instance.invoke = AsyncMock(return_value={
                "flights": [
                    {
                        "airline": "Air France",
    ...
    async def test_memory_update_node(self, mock_config, sample_state):
        ...
        with patch.object(MemoryUpdateNode, '_update_knowledge_graph') as mock_kg_update:
            with patch.object(MemoryUpdateNode, '_update_session_data') as mock_session_update:
    ...
    async def test_graph_routing_logic(self, mock_config, sample_state):
        ...
        test_cases = [
            ("router", ["flight_agent", "accommodation_agent", "budget_agent", "itinerary_agent"]),
        ...
        for current_agent, expected_next in test_cases:
            ...
            assert next_node in expected_next, f"Expected {next_node} to be in {expected_next} for agent {current_agent}"
...
class TestErrorHandling:
    ...
    async def test_mcp_connection_error(self, mock_config, sample_state):
        ...
        with patch('tripsage.mcp_abstraction.manager.MCPManager') as mock_manager:
            ...
            mock_manager_instance = Mock()
            mock_manager_instance.invoke = AsyncMock(side_effect=Exception("MCP connection failed"))
            mock_manager.return_value = mock_manager_instance
...
class TestLangGraphIntegration:
    ...
    async def test_error_recovery_flow(self, mock_config, sample_state):
        ...
        with patch('tripsage.mcp_abstraction.manager.MCPManager') as mock_manager:
            # Mock MCP error
            mock_manager_instance = Mock()
            mock_manager_instance.invoke = AsyncMock(side_effect=Exception("Service error"))
            mock_manager.return_value = mock_manager_instance
