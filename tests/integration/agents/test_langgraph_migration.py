...
class TestLangGraphMigration:
    ...
    async def test_router_node_classification(self):
        ...
        with patch.object(router, "classifier") as mock_classifier:
            ...
            state = create_initial_state(
                "test_user", "I need flights from New York to Los Angeles"
            )
            ...
            assert (
                "flight"
                in updated_state["handoff_context"]["routing_reasoning"].lower()
            )
    ...
    async def test_router_node_fallback(self):
        ...
        with patch.object(router, "classifier") as mock_classifier:
            ...
            assert updated_state["handoff_context"]["routing_confidence"] == 0.3
    ...
    async def test_session_continuity(self, orchestrator):
        ...
        result1 = await orchestrator.process_message(
            user_id, "I want to plan a trip to Paris", session_id
        )

        # Second message in same session
        result2 = await orchestrator.process_message(
            user_id, "What's the weather like there?", session_id
        )
    ...
    def test_mcp_tool_agent_mapping(self, mock_mcp_manager):
        ...
        assert "google_maps_geocode" in flight_tool_names
        ...
        assert "accommodations_search_stays" in accommodation_tool_names
        assert "google_maps_geocode" in accommodation_tool_names
    ...
    async def test_mcp_tool_wrapper_execution(self, mock_mcp_manager):
        ...
        tool = MCPToolWrapper("test_service", "test_method", "Test tool description")
        ...
        mock_mcp_manager.invoke.assert_called_once_with(
            "test_service", "test_method", {"param1": "value1", "param2": "value2"}
        )
    ...
    async def test_performance_improvement_indicators(self, orchestrator):
        ...
        # Should complete 5 messages in reasonable time
        # (< 10 seconds for stub implementation)
