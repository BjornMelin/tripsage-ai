...
class TestConfig:
    ...
    # Schema validation
    REQUIRED_TABLES = [
        'trips', 'trip_collaborators', 'flights', 'accommodations',
        'chat_sessions', 'chat_messages', 'memories', 'session_memories',
        'api_keys'
    ]
...
class MockSupabaseClient:
    ...
    async def _select_collaborators(self, query: str, params: tuple) -> List[Dict[str, Any]]:
        ...
        for collab_key, collab_data in self.data_store['trip_collaborators'].items():
            ...
            # User owns the trip
            elif (trip_id in self.data_store['trips'] and 
                  self.data_store['trips'][trip_id]['user_id'] == str(self.current_user_id)):
                accessible_collabs.append(collab_data)
    ...
    async def _select_memories(self, query: str, params: tuple) -> List[Dict[str, Any]]:
        ...
        for memory_id, memory_data in self.data_store['memories'].items():
    ...
    async def _insert_trip(self, params: tuple) -> None:
        ...
        if self.constraints_enabled and not self._user_exists(user_id):
            raise Exception(f'Foreign key constraint "trips_user_id_fkey" violated')
    ...
    async def _insert_collaborator(self, params: tuple) -> None:
        ...
        if self.constraints_enabled:
            if trip_id not in self.data_store['trips']:
                raise Exception(f'Foreign key constraint "trip_collaborators_trip_id_fkey" violated')
            if not self._user_exists(user_id):
                raise Exception(f'Foreign key constraint "trip_collaborators_user_id_fkey" violated')
    ...
    async def _insert_memory(self, params: tuple) -> None:
        ...
        # FK validation: user must exist
        if self.constraints_enabled and not self._user_exists(user_id):
            raise Exception(f'Foreign key constraint "memories_user_id_fkey" violated')
    ...
    def _user_exists(self, user_id: UUID) -> bool:
        """Check if user exists (simplified - in real implementation would check auth.users)."""
...
def schema_files():
    ...
    migration_files = ['20250610_01_fix_user_id_constraints.sql', '20250609_02_consolidated_production_schema.sql']
...
async def populated_database(mock_supabase_client, test_users, test_trips):
    ...
    # Add test trips
    for trip_name, trip in test_trips.items():
        ...
        for collab_data in trip.collaborators.values():
            await client.execute_sql(
                ...
                (trip.id, collab_data['user'].id, collab_data['permission_level'], trip.owner.id)
            )
    ...
    def get_summary():
        return {
            ...
            'performance_violations': [
                q for q in metrics['query_times'] 
...
def assert_rls_isolation(user1_data: List[Any], user2_data: List[Any], user1_id: UUID, user2_id: UUID):
    ...
    for item in user1_data:
        if hasattr(item, 'user_id'):
            assert str(item.user_id) == str(user1_id), f"User1 data contains data from another user"
    ...
    for item in user2_data:
        if hasattr(item, 'user_id'):
            assert str(item.user_id) != str(user1_id), f"User2 can see User1's data - RLS violation"
        elif isinstance(item, dict) and 'user_id' in item:
            assert str(item['user_id']) != str(user1_id), f"User2 can see User1's data - RLS violation"
