...
def valid_jwt_token():
    ...
    return jwt.encode(payload, settings.database.supabase_jwt_secret.get_secret_value(), algorithm="HS256")
...
class TestWebSocketAuthentication:
    ...
    async def test_successful_authentication(self, mock_ws_manager, mock_websocket, valid_jwt_token):
        ...
        mock_ws_manager.authenticate_connection = AsyncMock(return_value=MagicMock(
            success=True,
...
class TestWebSocketAgentStatus:
    ...
    async def test_agent_status_connection(self, mock_ws_manager, mock_websocket, valid_jwt_token):
        ...
        auth_data = {
            "token": valid_jwt_token,
            "channels": [f"agent_status:{user_id}"]
        }
        mock_websocket.receive_text = AsyncMock(return_value=json.dumps(auth_data))
...
class TestWebSocketSubscriptions:
    ...
    async def test_channel_subscription(self, mock_ws_manager):
        ...
        mock_ws_manager.subscribe_connection = AsyncMock(return_value=MagicMock(
            success=True,
    ...
    async def test_channel_unsubscription(self, mock_ws_manager):
        ...
        subscribe_request = WebSocketSubscribeRequest(
            channels=[],
            unsubscribe_channels=["old_channel1", "old_channel2"]
        )
...
class TestWebSocketPerformance:
    ...
    async def test_message_streaming_simulation(self, mock_ws_manager):
        ...
        message_data = {"content": "This is a longer message that will be broken into chunks for streaming."}
    ...
    async def test_typing_indicators(self, mock_ws_manager):
        ...
        typing_start_events = [e for e in sent_events if hasattr(e, 'type') and e.type == WebSocketEventType.CHAT_TYPING_START]
        typing_stop_events = [e for e in sent_events if hasattr(e, 'type') and e.type == WebSocketEventType.CHAT_TYPING_STOP]
