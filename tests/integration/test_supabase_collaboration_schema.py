...
class MockDatabaseService:
    ...
    def _get_column_info(self) -> List[Dict[str, Any]]:
        ...
        return [
            ...
            {"table_name": "session_memories", "column_name": "user_id", "data_type": "uuid"},
...
class TestRLSPolicyValidation:
    ...
    async def test_permission_level_inheritance(self, mock_db_service, test_users):
        """Test that trip-related data inherits proper permissions from trip collaboration."""
        editor = test_users['editor']
        mock_db_service.set_current_user(editor.id)
...
class TestDatabaseFunctions:
    ...
    async def test_get_user_accessible_trips_function(self, mock_db_service, test_users):
        ...
        trips = await mock_db_service.fetch_all(
            "SELECT * FROM get_user_accessible_trips($1, true)",
            user.id
        )
...
class TestMigrationCompatibility:
    ...
    async def test_migration_data_preservation(self, mock_db_service):
        ...
        mock_db_service.fetch_all = AsyncMock(return_value=[
            {"id": 1, "user_id": "00000000-0000-0000-0000-000000000001", "content": "Existing memory"}
        ])
...
class TestMultiUserScenarios:
    ...
    async def test_multiple_permission_levels_scenario(self, mock_db_service, test_users):
        ...
        mock_db_service.fetch_all = AsyncMock(return_value=[
            {"user_id": str(admin.id), "permission_level": "admin", "can_manage": True},
            {"user_id": str(editor.id), "permission_level": "edit", "can_manage": False},
            {"user_id": str(viewer.id), "permission_level": "view", "can_manage": False}
        ])
...
class TestDatabaseFixtures:
    ...
    async def test_test_data_isolation(self, clean_database, test_users):
        ...
        trip = await db.fetch_one(
            "SELECT * FROM trips WHERE id = $1", 1
        )
    ...
    async def test_collaboration_data_setup(self, sample_collaboration_data, mock_db_service):
        ...
        # Verify trip exists
        mock_db_service.fetch_one = AsyncMock(return_value={
            "id": data['trip_id'],
        ...
        mock_db_service.fetch_all = AsyncMock(return_value=[
            {"user_id": str(data['collaborators']['editor']['user'].id), "permission_level": "edit"},
            {"user_id": str(data['collaborators']['viewer']['user'].id), "permission_level": "view"}
        ])
