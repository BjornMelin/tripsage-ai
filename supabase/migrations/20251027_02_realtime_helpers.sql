...
create index if not exists idx_chat_sessions_id on public.chat_sessions (id);
create index if not exists idx_trips_id_user on public.trips (id, user_id);
create index if not exists idx_trip_collaborators_trip_user on public.trip_collaborators (trip_id, user_id);
