...
const calculateDealsStats = (deals: Deal[]): DealStats => {
  ...
  const avgDiscount = dealsWithDiscount > 0 ? totalDiscountPercentage / dealsWithDiscount : 0;
...
const matchDealWithFilters = (deal: Deal, filters?: DealState["filters"]): boolean => {
  ...
  if (filters.minDiscount !== undefined && filters.minDiscount > 0) {
    const discount = deal.discountPercentage || 
      (deal.originalPrice ? calculateDiscountPercentage(deal.originalPrice, deal.price) : 0);
    ...
    (set, get) => ({
      ...
      removeDeal: (id) => 
        set((state) => {
          ...
          const newRecentlyViewedDeals = recentlyViewedDeals.filter(dealId => dealId !== id);
      ...
      // Featured deals
      setFeaturedDeals: (dealIds) => 
        set({ featuredDeals: dealIds }),
