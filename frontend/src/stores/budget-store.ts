...
const calculateBudgetSummary = (budget: Budget, expenses: Expense[]): BudgetSummary => {
  ...
  if (startDate && endDate) {
    ...
    const elapsedDays = Math.max(1, Math.ceil((new Date().getTime() - startDate.getTime()) / (1000 * 60 * 60 * 24)));
    
    dailyAverage = elapsedDays > 0 ? totalSpent / elapsedDays : 0;
    dailyLimit = daysRemaining && daysRemaining > 0 ? totalRemaining / daysRemaining : 0;
    ...
    (set, get) => ({
      ...
      addBudget: (budget) => 
        set((state) => {
          ...
          return {
            ...
            activeBudgetId: state.activeBudgetId === null ? newBudget.id : state.activeBudgetId,
          };
        }),
      ...
      removeBudget: (id) => 
        set((state) => {
          ...
          const newActiveBudgetId = state.activeBudgetId === id ? null : state.activeBudgetId;
