...
import { useMemoryContext, useMemoryInsights, useMemoryStats } from '../../../lib/hooks/use-memory';
...
describe('MemoryContextPanel', () => {
  ...
  it('renders memory context panel with user data', async () => {
    ...
    render(
      <MemoryContextPanel userId="user-123" />,
  ...
  it('shows loading state when data is loading', () => {
    mockUseMemoryContext.mockReturnValue({ ...mockMemoryContext, isLoading: true });
    mockUseMemoryInsights.mockReturnValue({ ...mockMemoryInsights, isLoading: true });
  ...
  it('displays AI recommendations in insights tab', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('Consider luxury hotels in Kyoto for your next trip')).toBeInTheDocument();
      expect(screen.getByText('Book flights 6-8 weeks in advance for best prices')).toBeInTheDocument();
    });
  });
  ...
  it('updates when userId prop changes', async () => {
    ...
    // Change userId
    rerender(
      <MemoryContextPanel userId="user-456" />,
    );
  ...
  it('can be disabled via enabled prop', () => {
    ...
    render(
      <MemoryContextPanel userId="user-123" enabled={false} />,
      { wrapper: createWrapper() }
    );
