...
describe('useDeals Hook', () => {
  ...
  it('should provide access to recently viewed deals', () => {
    ...
    expect(result.current.recentlyViewedDeals.map(d => d.id)).toContain(sampleDeals[0].id);
    expect(result.current.recentlyViewedDeals.map(d => d.id)).toContain(sampleDeals[2].id);
  });
  ...
  it('should sort deals by discount', () => {
    ...
    const sortedDeals = result.current.sortDeals(result.current.allDeals, 'discount', 'desc');
  ...
  it('should sort deals by price', () => {
    ...
    const sortedDeals = result.current.sortDeals(result.current.allDeals, 'price', 'asc');
...
describe('useDealAlerts Hook', () => {
  ...
  it('should toggle alert active state', () => {
    ...
    expect(result.current.alerts.find(a => a.id === sampleAlerts[0].id)?.isActive).toBe(false);
