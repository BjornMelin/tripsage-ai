...
const PASSWORD_SCHEMA = refinedSchemas.strongPassword;
...
const FUTURE_DATE_SCHEMA = DATE_SCHEMA.refine((date) => new Date(date) > new Date(), {
...
export const apiKeyFormSchema = z.object({
  description: z.string().max(200, { error: "Description too long" }).optional(),
  key: z
    .string()
    .min(1, { error: "API key is required" })
    .max(500, { error: "API key too long" })
    .regex(/^[a-zA-Z0-9\-_.]+$/, { error: "Invalid API key format" }),
  name: z
    .string()
    .min(1, { error: "Name is required" })
