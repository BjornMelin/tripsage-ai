...
import { StateDepartmentProvider } from "../providers/state-department";
...
describe("StateDepartmentProvider", () => {
  ...
  beforeEach(() => {
    // Create fresh provider instance for each test to ensure isolation
  ...
  describe("getCountryAdvisory", () => {
    const mockAdvisoryResponse: Array<{
    ...
    test("fetches and normalizes advisory for valid country code", async () => {
      ...
      expect(result?.overallScore).toBe(85); // Level 1 = 85
    ...
    test("normalizes Level 2 advisory correctly", async () => {
      ...
      expect(result).not.toBeNull();
      expect(result?.overallScore).toBe(60); // Level 2 = 60
      expect(result?.categories.length).toBeGreaterThan(0);
    });
    ...
    test("extracts categories from summary", async () => {
      ...
      expect(result?.categories.length).toBeGreaterThan(0);
      // Should have terrorism and crime categories
    ...
    test("finds advisory by direct category match", async () => {
      // Create fresh provider to avoid cache interference
      const freshProvider = new StateDepartmentProvider();
      const advisoryWithDirectCode = [
        {
          Category: ["US"], // Direct ISO code match
          id: "us-advisory",
          Link: "https://travel.state.gov/...",
          Published: "2024-01-01T00:00:00-05:00",
    ...
    test("handles timeout errors without cache", async () => {
      // Create fresh provider to ensure no cache interference
      const freshProvider = new StateDepartmentProvider();
      // No cache set, so timeout should result in null
    ...
    test("handles timeout errors with stale cache", async () => {
      ...
      // Second call times out, should use stale cache
