 ...
 * compliance. Firecrawl may return additional fields (e.g., content, score,
 * source) that are not part of our strict output schema.
 *
 * This tool-specific normalizer provides type safety and performance. Other tools
 * that need normalization should follow the same pattern: extract only schema-defined
...
export function normalizeWebSearchResults(items: unknown[]): WebSearchSource[] {
  ...
  return items
    ...
    .filter((r): r is WebSearchSource => r !== null);
}
