...
function buildAccommodationTools(identifier: string): ToolSet {
	...
	const searchAccommodations = createAiTool({
		...
		execute: (params, callOptions) =>
			invokeTool(searchTool, params, callOptions),
		guardrails: {
			cache: {
				hashInput: true,
	...
	const getAccommodationDetails = createAiTool({
		...
		execute: (params, callOptions) =>
			invokeTool(detailsTool, params, callOptions),
		guardrails: {
			rateLimit: {
				errorCode: TOOL_ERROR_CODES.toolRateLimited,
				identifier: () => rateLimit.identifier,
	...
	const checkAvailability = createAiTool({
		description:
			availabilityTool.description ?? "Check accommodation availability",
		execute: (params, callOptions) =>
			invokeTool(availabilityTool, params, callOptions),
		guardrails: {
			rateLimit: {
				errorCode: TOOL_ERROR_CODES.toolRateLimited,
				identifier: () => rateLimit.identifier,
	...
	const bookAccommodation = createAiTool({
		...
		execute: (params, callOptions) =>
			invokeTool(bookingTool, params, callOptions),
		guardrails: {
			rateLimit: {
				errorCode: TOOL_ERROR_CODES.toolRateLimited,
				identifier: () => rateLimit.identifier,
