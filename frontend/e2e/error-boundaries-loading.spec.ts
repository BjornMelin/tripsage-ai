...
test.describe('Error Boundaries and Loading States', () => {
  ...
  test.describe('Global Error Boundary', () => {
    test('should catch critical application errors', async ({ page }) => {
      ...
      await expect(page.locator('text=Critical Error')).toBeVisible({ timeout: 5000 })
      await expect(page.locator('text=The application encountered a critical error')).toBeVisible()
    })
  })
  ...
  test.describe('Performance', () => {
    test('should not create memory leaks with frequent error/loading state changes', async ({ page }) => {
    ...
    test('should render loading skeletons efficiently', async ({ page }) => {
      ...
      await expect(page.locator('[role="status"]').first()).toBeVisible({ timeout: 1000 })
