...
test.describe("Dashboard Functionality", () => {
	...
	test("dashboard page renders correctly after authentication", async ({
		page,
	}) => {
		...
		await expect(
			page.getByText("Welcome to TripSage AI")
		).toBeVisible();
	...
	test("dashboard navigation works correctly", async ({ page }) => {
		...
		await page.getByRole("navigation").getByRole("link", { name: "AI Assistant" }).click();
	...
	test("theme toggle works", async ({ page }) => {
		...
		const headerThemeToggle = page.getByRole("banner").getByRole("button", { name: "Toggle theme" });
