...
class OpenWeatherMapClient:
    ...
    def _get_api_key(self) -> str:
        ...
        if hasattr(config, 'weather_mcp') and hasattr(config.weather_mcp, 'openweathermap_api_key'):
    ...
    async def _make_request(self, endpoint: str, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            raise APIError(
                message=f"OpenWeatherMap API error: {e.response.status_code} - {error_info.get('message', '')}",
    ...
    async def get_current_weather(
        ...
        if location.lat is not None and location.lon is not None:
            ...
            params["q"] = location.city if not location.country else f"{location.city},{location.country}"
        else:
            raise ValueError("Either coordinates (lat, lon) or city must be provided")
