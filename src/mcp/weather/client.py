...
class WeatherMCPClient(FastMCPClient):
    ...
    def __init__(
        ...
        """
        if endpoint is None:
            endpoint = config.weather_mcp.endpoint if hasattr(config, 'weather_mcp') else "http://localhost:8003"
        
        api_key = api_key or (config.weather_mcp.api_key if hasattr(config, 'weather_mcp') else None)
    ...
    async def get_forecast(
        ...
        try:
            ...
            raise MCPError(
                ...
                server=self.server_name,
                tool="get_forecast",
                params={"location": {"city": city, "country": country, "lat": lat, "lon": lon}, "days": days}
            )
...
class WeatherService:
    ...
    async def get_trip_weather_summary(
        ...
        try:
            ...
            # Get forecast data
            forecast = await self.client.get_forecast(city=city, country=country, days=16)
            ...
            most_common = max(weather_counts.items(), key=lambda x: x[1]) if weather_counts else None
            ...
            return TravelWeatherSummary(
                ...
                conditions={
                    ...
                    "frequency": most_common[1] / len(trip_days) if most_common and trip_days else None,
    ...
    async def compare_destinations_weather(
        ...
        try:
            ...
            for destination in destinations:
                ...
                try:
                    if date:
                        ...
                        forecast = await self.client.get_forecast(city=city, country=country)
                        for day in forecast["daily"]:
                            if day["date"] == date:
                                results.append({
                                    "destination": destination,
                        ...
                        # Get current weather
                        weather = await self.client.get_current_weather(city=city, country=country)
                        results.append({
                            "destination": destination,
            ...
            if valid_results:
                ...
                temp_sorted = sorted(
                    valid_results,
                    key=lambda x: x["temperature"] if date is None else x["temperature"]["average"],
                    reverse=True
                )
    ...
    async def get_optimal_travel_time(
        ...
        try:
            ...
            activity_recs = [rec for rec in recommendation["recommendations"]["activities"] 
            ...
            if activity_recs:
                ...
                activity_advice = "No specific recommendations available for this activity type."
