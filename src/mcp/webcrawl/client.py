...
class WebCrawlMCPClient(BaseMCPClient):
    ...
    def __init__(self):
        ...
        super().__init__(
            ...
            api_key=self.settings.webcrawl_mcp.firecrawl_api_key.get_secret_value() 
                if self.settings.webcrawl_mcp.firecrawl_api_key else None
        )
        
        # Crawl4AI configuration
        self.crawl4ai_api_key = self.settings.webcrawl_mcp.crawl4ai_api_key.get_secret_value() if self.settings.webcrawl_mcp.crawl4ai_api_key else None
        self.crawl4ai_api_url = self.settings.webcrawl_mcp.crawl4ai_api_url
        
        # Firecrawl configuration
        self.firecrawl_api_key = self.settings.webcrawl_mcp.firecrawl_api_key.get_secret_value() if self.settings.webcrawl_mcp.firecrawl_api_key else None
    ...
    async def _call_validate_tool(
        self, tool_name: str, params: BaseModel, response_model: Type[T], 
        skip_cache: bool = False, cache_key: Optional[str] = None,
        ...
        try:
            ...
            raise MCPError(f"Invalid parameters for {tool_name}: {str(e)}")
        except Exception as e:
            logger.error(f"Error in {tool_name}: {str(e)}")
            raise MCPError(f"MCP error: {str(e)}")

    async def extract_page_content(
        self, url: str, selectors: Optional[List[str]] = None,
        ...
        try:
            return await self._call_validate_tool(
                ...
                ScrapeParams(url=url, options=ScrapeOptions(
            ...
            raise MCPError(f"Failed to extract page content: {str(e)}")
    
    async def search_destination_info(
        self, destination: str, topics: Optional[List[str]] = None,
        max_results: int = 5, traveler_profile: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            return await self._call_validate_tool(
                "mcp__webcrawl__firecrawl_search",
                params,
                SearchResponse
            )
        except Exception as e:
            raise MCPError(f"Failed to search destination info: {str(e)}")
    
    async def deep_research(
        self, query: str, max_depth: int = 3, max_urls: int = 10, 
        time_limit: int = 60
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            raise MCPError(f"Failed to conduct deep research: {str(e)}")
    
    async def monitor_price_changes(
        self, url: str, price_selector: str, frequency: str = "daily",
        ...
        try:
            ...
            raise MCPError(f"Failed to set up price monitoring: {str(e)}")
    ...
    async def get_latest_events(
        ...
        try:
            ...
            raise MCPError(f"Failed to get latest events: {str(e)}")
    
    async def crawl_travel_blog(
        self, destination: str, topics: List[str], max_blogs: int = 3,
        ...
        try:
            ...
            raise MCPError(f"Failed to crawl travel blogs: {str(e)}")
