...
class FlightsMCPConfig(ClientConfig):
    ...
    endpoint: str = Field(
        ..., 
        description="The Flights MCP server endpoint URL"
    )
    api_key: Optional[str] = Field(
        None, 
        description="Duffel API key for authentication"
    )
    timeout: float = Field(
        30.0, 
    ...
    use_cache: bool = Field(
        True, 
        description="Whether to use caching for Flights MCP requests"
    )
    cache_ttl: int = Field(
...
class FlightsMCPClientFactory(BaseClientFactory[FlightsMCPClient, FlightsMCPConfig]):
    ...
    def _load_config_from_settings(self) -> Dict[str, Any]:
        ...
        if hasattr(settings, "flights_mcp"):
            ...
            config = {
                "endpoint": getattr(flights_config, "endpoint", "http://localhost:3000"),
            }
