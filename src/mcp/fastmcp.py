...
This module provides enhanced MCP server and client implementations using the FastMCP 2.0
framework for consistency across all TripSage's MCP implementations.
"""
...
from ..utils.error_handling import MCPError, APIError, log_exception
...
class FastMCPTool:
    ...
    def create(
        ...
        tool_cls = type(
            ...
            (FastMCPTool,), 
            {
                "async_handler": staticmethod(handler) if asyncio.iscoroutinefunction(handler) else None,
                "sync_handler": staticmethod(handler) if not asyncio.iscoroutinefunction(handler) else None,
                "execute": lambda self, params: self.async_handler(params) if self.async_handler else asyncio.create_task(self.sync_handler(params)),
            }
        )
...
class FastMCPServer(BaseMCPServer):
    ...
    def _setup_openai_routes(self) -> None:
        ...
        async def openai_compat(request: OpenAICompatRequest):
            ...
            # In a real implementation, this would process the request and call the appropriate tools
...
class FastMCPClient(BaseMCPClient):
    ...
    async def call_tool_with_json(self, tool_name: str, json_input: str) -> Dict[str, Any]:
