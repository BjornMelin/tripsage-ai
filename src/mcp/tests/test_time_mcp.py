...
from ..time.api_client import TimeFormat, TimeZoneDatabase
...
class TestTimeZoneDatabase:
    ...
    async def test_convert_time(self, timezone_db):
        ...
        result = await timezone_db.convert_time(
            time_str="12:00", source_timezone="UTC", target_timezone="America/New_York"
        )
...
class TestTimeMCPClient:
    ...
    async def test_get_current_time(self, mock_client):
        ...
        mock_client.call_tool.assert_called_once_with(
            "get_current_time", {"timezone": "America/New_York"}, skip_cache=False
        )
...
class TestTimeService:
    ...
    async def test_get_local_time(self, mock_service):
        ...
        mock_service.client.get_current_time.assert_called_once_with(
            timezone="Asia/Tokyo"
        )
        assert result == mock_response
