...
from ..weather.api_client import OpenWeatherMapClient, WeatherLocation, CurrentWeather, WeatherForecast
...
class TestWeatherLocation:
    ...
    def test_with_all_fields(self):
        ...
        location = WeatherLocation(city="London", country="GB", lat=51.5074, lon=-0.1278)
        assert location.city == "London"
        assert location.country == "GB"
        assert location.lat == 51.5074
        assert location.lon == -0.1278
...
class TestOpenWeatherMapClient:
    ...
    def mock_owm_client(self):
        ...
        with patch("src.mcp.weather.api_client.OpenWeatherMapClient._get_api_key", return_value="fake_api_key"):
...
class TestWeatherMCPClient:
    ...
    async def test_get_travel_recommendation(self, mock_client):
        ...
        mock_response = {
            ...
            "forecast": {
                "daily": [
                    {"date": "2025-05-10", "temp_avg": 15.5, "weather": {"main": "Clear"}}
                ]
            },
            "recommendations": {
...
class TestWeatherService:
    ...
    async def test_get_destination_weather(self, mock_service):
        ...
        mock_service.client.get_current_weather.assert_called_once_with(city="London", country="GB")
        assert result == mock_response
    ...
    async def test_get_trip_weather_summary(self, mock_service):
        ...
        mock_service.client.get_forecast.assert_called_once_with(city="London", country="GB", days=16)
    ...
    async def test_compare_destinations_weather(self, mock_service):
        ...
        assert result.ranking == ["Cancun, MX", "Miami, US"]  # Sorted by temperature (higher first)
