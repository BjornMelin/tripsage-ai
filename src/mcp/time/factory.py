...
class TimeMCPConfig(ClientConfig):
    ...
    endpoint: str = Field(..., description="The Time MCP server endpoint URL")
    api_key: Optional[str] = Field(
        None, description="API key for Time MCP server authentication"
    )
    timeout: float = Field(
        30.0, description="Request timeout in seconds", ge=1.0, le=120.0
    )
    use_cache: bool = Field(
        True, description="Whether to use caching for Time MCP requests"
    )
    cache_ttl: int = Field(
