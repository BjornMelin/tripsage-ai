...
class TimeMCPClient(FastMCPClient):
    ...
    def __init__(
        ...
        """
        if endpoint is None:
            endpoint = config.time_mcp.endpoint if hasattr(config, 'time_mcp') else "http://localhost:8004"
        
        api_key = api_key or (config.time_mcp.api_key if hasattr(config, 'time_mcp') else None)
    ...
    async def get_current_time(
        ...
        """
        try:
            return await self.call_tool("get_current_time", {"timezone": timezone}, skip_cache=skip_cache)
    ...
    async def convert_time(
        ...
        try:
            ...
            raise MCPError(
                ...
                server=self.server_name,
                tool="convert_time",
                params={"time": time, "source_timezone": source_timezone, "target_timezone": target_timezone}
            )
    ...
    async def calculate_travel_time(
        ...
        try:
            ...
            return await self.call_tool("calculate_travel_time", params, skip_cache=skip_cache)
        except Exception as e:
    ...
    @redis_cache.cached("time_timezones", 86400)  # 24 hours (timezones don't change often)
