...
def get_config() -> Dict[str, Any]:
    ...
    return {
        ...
        "SUPABASE_SERVICE_ROLE_KEY": settings.database.supabase_service_role_key.get_secret_value()
        ...
        "WEATHER_MCP_API_KEY": settings.weather_mcp.api_key.get_secret_value()
        if settings.weather_mcp.api_key
        else None,
        "OPENWEATHERMAP_API_KEY": settings.weather_mcp.openweathermap_api_key.get_secret_value(),
        "VISUAL_CROSSING_API_KEY": settings.weather_mcp.visual_crossing_api_key.get_secret_value()
        if settings.weather_mcp.visual_crossing_api_key
        else None,
        # Web Crawl MCP
        "WEBCRAWL_MCP_ENDPOINT": settings.webcrawl_mcp.endpoint,
        "WEBCRAWL_MCP_API_KEY": settings.webcrawl_mcp.api_key.get_secret_value()
        if settings.webcrawl_mcp.api_key
        else None,
        # Browser MCP
        "BROWSER_MCP_ENDPOINT": settings.browser_mcp.endpoint,
        "BROWSER_MCP_API_KEY": settings.browser_mcp.api_key.get_secret_value()
        if settings.browser_mcp.api_key
        else None,
        # Flights MCP
        "FLIGHTS_MCP_ENDPOINT": settings.flights_mcp.endpoint,
        "FLIGHTS_MCP_API_KEY": settings.flights_mcp.api_key.get_secret_value()
        if settings.flights_mcp.api_key
        else None,
        ...
        "GOOGLE_MAPS_MCP_API_KEY": settings.google_maps_mcp.api_key.get_secret_value()
        if settings.google_maps_mcp.api_key
        else None,
        "GOOGLE_MAPS_API_KEY": settings.google_maps_mcp.maps_api_key.get_secret_value(),
        # Time MCP
        "TIME_MCP_ENDPOINT": settings.time_mcp.endpoint,
        "TIME_MCP_API_KEY": settings.time_mcp.api_key.get_secret_value()
        if settings.time_mcp.api_key
        else None,
        # Memory MCP
        "MEMORY_MCP_ENDPOINT": settings.memory_mcp.endpoint,
        "MEMORY_MCP_API_KEY": settings.memory_mcp.api_key.get_secret_value()
        if settings.memory_mcp.api_key
        else None,
        # Sequential Thinking MCP
        "SEQ_THINKING_MCP_ENDPOINT": settings.sequential_thinking_mcp.endpoint,
        "SEQ_THINKING_MCP_API_KEY": settings.sequential_thinking_mcp.api_key.get_secret_value()
        if settings.sequential_thinking_mcp.api_key
        else None,
    }
...
class _WeatherMCPConfig:
    ...
    visual_crossing_api_key = (
        settings.weather_mcp.visual_crossing_api_key.get_secret_value()
        if settings.weather_mcp.visual_crossing_api_key
        else None
    )
