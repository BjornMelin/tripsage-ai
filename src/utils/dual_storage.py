...
async def store_trip_with_dual_storage(
    ...
    for acc_name in accommodation_names:
        ...
        for accommodation in accommodations:
            if accommodation.get("name") == acc_name and accommodation.get("destination"):
                relations.append({
                    "from": acc_name,
...
async def retrieve_trip_with_dual_storage(
    ...
    logger.info(f"Retrieving unstructured trip data from Neo4j via Memory MCP for trip {trip_id}")
    await memory_client.initialize()
    ...
    if include_graph:
        ...
        if search_result:
            node_names = [node.get("name") for node in search_result if node.get("name")]
...
async def update_trip_with_dual_storage(
    ...
    logger.info(f"Updating unstructured trip data in Neo4j via Memory MCP for trip {trip_id}")
    await memory_client.initialize()
    ...
    if not trip_nodes:
        ...
        if "description" in trip_data:
            ...
            observations = [
                f"Trip from {trip_data.get('start_date', db_trip.get('start_date'))} to {trip_data.get('end_date', db_trip.get('end_date'))}",
...
async def delete_trip_with_dual_storage(trip_id: str) -> Dict[str, Any]:
    ...
    logger.info(f"Deleting unstructured trip data from Neo4j via Memory MCP for trip {trip_id}")
    await memory_client.initialize()
