...
relationships/unstructured data are stored in a graph database (e.g., Neo4j via Memory MCP).
"""
...
class DualStorageService(Generic[P, G], metaclass=abc.ABCMeta):
    ...
    async def retrieve(
        ...
        logger.info(f"Retrieving structured {self.entity_type} data from primary database")
        primary_data = await self._retrieve_from_primary(entity_id)
        
        if not primary_data:
            logger.warning(f"{self.entity_type} with ID {entity_id} not found in primary database")
