...
async def validate_and_call_mcp_tool(
    ...
    mcp_url = f"{endpoint}/mcp__supabase__{tool_name}"
    ...
    try:
        ...
        logger.error(f"Error calling Supabase MCP tool {tool_name}: {str(e)}")
        raise MCPError(
            message=f"Failed to execute Supabase tool {tool_name}: {str(e)}",
            server="Supabase",
            tool=tool_name,
            params=params,
        ) from e
