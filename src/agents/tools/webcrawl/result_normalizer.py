...
class ResultNormalizer:
    ...
    def normalize_price_result(
        ...
        """Normalize a price monitoring result from any source into a standardized format.
        ...
        for i, item in enumerate(normalized["items"]):
            # Get original item
            original_item = result.get("items", [])[i] if i < len(result.get("items", [])) else {}
    ...
    def _generate_formatted_summary(self, result: Dict[str, Any]) -> str:
        ...
        return f"Successfully crawled {url} and found {len(items)} content blocks " \
