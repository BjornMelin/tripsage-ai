...
@function_tool
async def search_destination_info_tool(params: SearchDestinationParams) -> Dict[str, Any]:
...
async def monitor_price_changes_tool(params: PriceMonitorParams) -> Dict[str, Any]:
    ...
    try:
        ...
        price_selector = next(iter(params.target_selectors.values())) if params.target_selectors else ".price"
...
def extract_destination_from_url(url: str) -> str:
    ...
    common_destinations = [
        ...
        "bali", "bangkok", "istanbul", "dubai", "singapore", 
        "barcelona", "venice", "prague", "amsterdam", "athens",
        "hawaii", "kyoto", "cairo", "marrakech", "lisbon"
    ]
    
    # Check for destinations in URL
    for destination in common_destinations:
        if destination.replace(" ", "-") in url_lower or destination.replace(" ", "") in url_lower:
