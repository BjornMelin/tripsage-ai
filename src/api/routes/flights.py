...
async def verify_trip_access(trip_id: str, user_id: str):
    ...
    if response.data[0]["user_id"] != user_id:
        # Check if admin
        admin_response = (
            supabase.from_("users").select("is_admin").eq("id", user_id).execute()
        )
...
async def get_flight(
    flight_id: str, current_user: User = Depends(get_current_active_user)
):
    supabase = get_supabase_client()
...
async def create_flight(
    flight: FlightCreate, current_user: User = Depends(get_current_active_user)
):
    # Verify access to trip
    ...
    if not response.data:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Could not create flight"
        )
...
async def update_flight(
    ...
    update_response = (
        supabase.from_("flights").update(update_data).eq("id", flight_id).execute()
    )

    if not update_response.data:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Could not update flight"
        )
