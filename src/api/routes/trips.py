...
async def get_trips(current_user: User = Depends(get_current_active_user)):
    ...
    response = supabase.from_("trips").select("*").eq("user_id", current_user.id).execute()
...
async def get_trip(trip_id: str, current_user: User = Depends(get_current_active_user)):
    ...
    if response.data[0]["user_id"] != current_user.id:
        # Admin check
        admin_response = supabase.from_("users").select("is_admin").eq("id", current_user.id).execute()
...
async def create_trip(trip: TripCreate, current_user: User = Depends(get_current_active_user)):
    trip_data = trip.dict()
...
async def update_trip(
    ...
    update_response = supabase.from_("trips").update(update_data).eq("id", trip_id).execute()
    
    if not update_response.data:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Could not update trip")
    
    return update_response.data[0]

# Delete trip
@router.delete("/{trip_id}", status_code=status.HTTP_204_NO_CONTENT)
async def delete_trip(trip_id: str, current_user: User = Depends(get_current_active_user)):
    # Check if trip exists and user has access
    supabase = get_supabase_client()
    response = supabase.from_("trips").select("*").eq("id", trip_id).execute()
