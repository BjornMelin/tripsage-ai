...
async def get_users(current_user: User = Depends(get_current_active_user)):
    ...
    response = supabase.from_("users").select("id, email, full_name, is_admin").eq("id", current_user.id).execute()
...
async def get_user(user_id: str, current_user: User = Depends(get_current_active_user)):
    ...
    if current_user.id != user_id:
        # Check if admin
        supabase = get_supabase_client()
        response = supabase.from_("users").select("is_admin").eq("id", current_user.id).execute()
        if not response.data or not response.data[0].get("is_admin", False):
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Not authorized")
    
    supabase = get_supabase_client()
    response = supabase.from_("users").select("id, email, full_name").eq("id", user_id).execute()
