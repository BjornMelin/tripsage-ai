...
class DatabaseProvider(ABC):
    ...
    @abstractmethod
    async def execute_sql(self, query: str, params: Optional[Dict[str, Any]] = None) -> Any:
        """
...
class SupabaseProvider(DatabaseProvider):
    ...
    def __init__(self, url: str, key: str, options: Dict[str, Any] = None):
        ...
        # Log initialization
        logger.debug(f"Initialized Supabase provider with URL: {url}, options: {self.options}")
    ...
    async def connect(self) -> None:
        ...
        try:
            ...
            # Using a table that's likely to exist - you may want to adjust this depending
            # on your schema
            try:
                ...
                response = self.client.rpc("exec_sql", {"query": "SELECT 1"}).execute()
...
class NeonProvider(DatabaseProvider):
    ...
    async def connect(self) -> None:
        ...
        try:
            ...
            self.pool = await asyncpg.create_pool(
                ...
                server_settings={"application_name": "tripsage_app"}  # Mark connections for easier identification
            )
            ...
            self._connected = True
            logger.info(f"Connected to Neon successfully (pool size: {self.min_size}-{self.max_size}, inactive timeout: {self.max_inactive_connection_lifetime}s)")
        except Exception as e:
            stack_trace = traceback.format_exc()
    ...
    class NeonQueryBuilder(BaseQueryBuilder):
        ...
        def select(self, columns: str) -> "NeonQueryBuilder":
            return super().select(columns)

        def eq(self, column: str, value: Any) -> "NeonQueryBuilder":
            return super().eq(column, value)

        def neq(self, column: str, value: Any) -> "NeonQueryBuilder":
            return super().neq(column, value)

        def gt(self, column: str, value: Any) -> "NeonQueryBuilder":
            return super().gt(column, value)

        def lt(self, column: str, value: Any) -> "NeonQueryBuilder":
            return super().lt(column, value)

        def gte(self, column: str, value: Any) -> "NeonQueryBuilder":
            return super().gte(column, value)

        def lte(self, column: str, value: Any) -> "NeonQueryBuilder":
            return super().lte(column, value)

        def order(self, column: str, ascending: bool = True) -> "NeonQueryBuilder":
            return super().order(column, ascending)

        def limit(self, count: int) -> "NeonQueryBuilder":
            return super().limit(count)

        def offset(self, count: int) -> "NeonQueryBuilder":
            return super().offset(count)

        def range(self, from_val: int, to_val: int) -> "NeonQueryBuilder":
            return super().range(from_val, to_val)

        def insert(self, data: Union[Dict[str, Any], List[Dict[str, Any]]]) -> "NeonQueryBuilder":
            return super().insert(data)

        def update(self, data: Dict[str, Any]) -> "NeonQueryBuilder":
        ...
        async def _execute_update(self, conn: asyncpg.Connection) -> NeonQueryResult:
            ...
            if self._where_clauses:
                ...
                for clause in self._where_clauses:
                    # Extract the parameter number and increment it by the number of update params
                    parts = clause.split("$")
                    if len(parts) > 1:
                        param_num = int(parts[1])
                        adjusted_param_num = param_num + len(update_params)
                        adjusted_where_clauses.append(f"{parts[0]}${adjusted_param_num}")
                    else:
    ...
    async def execute_sql(self, query: str, params: Optional[Dict[str, Any]] = None) -> NeonQueryResult:
        """
        Execute a raw SQL query.
    ...
    async def execute_prepared_sql(self, query: str, params: List[Any] = None) -> NeonQueryResult:
        """
        Execute a prepared SQL statement.
    ...
    async def rpc(self, function_name: str, params: Dict[str, Any] = None) -> NeonQueryResult:
        """
        ...
        if param_names:
            param_pairs = [f"{name} => {placeholder}" 
                         for name, placeholder in zip(param_names, param_placeholders)]
        ...
        try:
            ...
            stack_trace = traceback.format_exc()
            logger.error(f"Failed to execute RPC function {function_name}: {e}\n{stack_trace}")
            raise QueryError(
