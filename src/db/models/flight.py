...
class Flight(BaseDBModel):
    ...
    departure_time: datetime = Field(..., description="Scheduled departure time with timezone")
    arrival_time: datetime = Field(..., description="Scheduled arrival time with timezone")
    ...
    segment_number: Optional[int] = Field(None, description="Segment number for multi-leg flights")
    search_timestamp: Optional[datetime] = Field(
        default_factory=datetime.now, 
        description="When this flight option was found"
    )
    booking_status: BookingStatus = Field(
        BookingStatus.VIEWED, 
        description="Status of the flight booking"
    )
    data_source: Optional[str] = Field(None, description="Source of the flight data (API provider)")
    ...
    def is_international(self) -> bool:
        ...
        if (len(self.origin) == 3 and len(self.destination) == 3 and 
            self.origin[0] != self.destination[0]):
            return True
        return False
    ...
    def to_dict(self, exclude_none: bool = True) -> Dict[str, Any]:
        ...
        if 'booking_status' in data and isinstance(data['booking_status'], BookingStatus):
