 ...
 * @returns `true` if `method` is one of POST, PUT, PATCH, or DELETE (case-insensitive), `false` otherwise.
 */
 ...
 *
 * Checks the provided request's cookie list first and falls back to the Next.js `cookies()` store when available.
 *
...
/**
 * Detects whether the request carries an Authorization header or Supabase authentication cookies.
 *
 * @returns An `AuthCredentialState` object: `hasAny` is `true` if either an Authorization header or Supabase auth cookies are present; `hasAuthorization` is `true` if an Authorization header is present; `hasCookie` is `true` if Supabase auth cookies are present.
 */
...
/**
 * Creates a higher-order wrapper that applies authorization, CSRF same-origin checks, BotID protection, rate limiting, request body parsing & validation, and telemetry around a route handler.
 *
 * The provided GuardsConfig controls which guards are applied (for example: `auth`, `botId`, `rateLimit`, `csrf`, `telemetry`, and `schema`) and how they behave. When enabled, authentication is validated before CSRF checks and bot protection; rate limiting is enforced early; request bodies are size-limited and validated against a Zod schema when provided; and handler execution is recorded with telemetry and fire-and-forget metrics.
 *
 * @param config - Configuration object that determines which guards to enable and their options.
 * @returns A function that accepts a RouteHandler and returns a Next.js route handler function that processes a NextRequest and RouteParamsContext and produces a Response.
 */
export function withApiGuards<SchemaType extends z.ZodType>(
