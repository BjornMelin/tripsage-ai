...
class SecurityValidator:
        ...
        for field_name, is_secure in secret_validation.items():
            if not is_secure:
                if settings.environment == "production":
                    ...
                    self.warnings.append(f"Weak secret detected for {field_name} (acceptable in {settings.environment})")
        
        # Check for fallback patterns in production
        if settings.environment == "production":
            for pattern in settings.FALLBACK_SECRET_PATTERNS:
                for field_name in ["secret_key", "database_service_key", "database_jwt_secret", "openai_api_key"]:
    ...
    def _report_results(self, settings: Settings) -> None:
        ...
        # Report issues
        if self.issues:
            logger.error(f"❌ {len(self.issues)} security issue(s) found:")
...
def generate_secure_config_template() -> str:
    ...
    try:
        ...
        security_template = f"""# TripSage Secure Configuration Template
...
"""
        ...
        logger.error(f"Failed to generate secure config template: {e}")
        return ""
