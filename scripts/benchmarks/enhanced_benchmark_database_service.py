...
class EnhancedDatabaseBenchmark:
    ...
    def calculate_statistics(self):
        ...
        for operation_name, metrics_list in operations.items():
            ...
            confidence_interval = (mean_duration - margin_error, mean_duration + margin_error)
    ...
    async def generate_csv_report(self, timestamp: str):
        ...
        with open(csv_file, 'w', newline='') as f:
            ...
            # Write header
            writer.writerow([
                'operation_name', 'duration_ms', 'success', 'memory_before_mb',
                'memory_after_mb', 'memory_peak_mb', 'cpu_percent', 'timestamp'
            ])
    ...
    def generate_console_report(self):
        ...
        print("-" * 100)
        print(f"{'Operation':<25} {'Mean(ms)':<10} {'P95(ms)':<10} {'P99(ms)':<10} {'Success%':<10} {'Ops/Sec':<10} {'Status':<10}")
        print("-" * 100)
        
        for name, stats in self.statistics.items():
            status = "REGRESSION" if stats.is_regression else "OK"
            print(f"{name:<25} {stats.mean_duration:<10.2f} {stats.p95_duration:<10.2f} "
        ...
        if self.regressions:
            ...
            print("-" * 80)
            for reg in self.regressions:
                print(f"⚠️  {reg.operation_name}: {reg.current_p95:.2f}ms "
