...
def check_hardcoded_secrets():
    ...
    try:
        ...
        cmd = [
            ...
            '--exclude-dir=build', '--exclude-dir=coverage', '--exclude-dir=.git',
            '--exclude-dir=__pycache__',
            '-E', '(password|secret|api_key|private_key).*=.*["\'][^"\']{8,}["\']',
            '.'
        ]
        ...
        if result.returncode == 0 and result.stdout.strip():
            ...
            for line in lines:
                ...
                log_finding("high", "Hardcoded Secrets", 
...
def check_sql_injection_protection():
    ...
    python_files = [f for f in glob.glob('**/*.py', recursive=True) 
                   if not any(exclude in f for exclude in ['node_modules', '.next', 'dist', 'build', '__pycache__', '.git'])]
    ...
    for file_path in python_files:
        try:
            ...
            for pattern in sql_patterns:
                ...
                if matches:
                    found_issues = True
                    log_finding("high", "SQL Injection", 
...
def check_xss_protection():
    ...
    frontend_files = glob.glob('frontend/src/**/*.ts', recursive=True) + \
    ...
    for file_path in frontend_files:
        try:
            ...
            for pattern in xss_patterns:
                ...
                if matches:
                    found_issues = True
                    log_finding("medium", "XSS Protection", 
...
def check_authentication_security():
    ...
    try:
        ...
        log_finding("critical", "Authentication", f"Authentication middleware not found: {auth_middleware_path}")
    except Exception as e:
    ...
    try:
        ...
        log_finding("critical", "Authentication", f"Session security service not found: {session_service_path}")
    except Exception as e:
        log_finding("medium", "Authentication", f"Error checking session service: {e}")
    
    log_finding("info", "Authentication", f"Authentication security: {checks_passed}/{total_checks} checks passed")
...
def check_cors_configuration():
    ...
    api_files = [f for f in glob.glob('tripsage/api/**/*.py', recursive=True) 
...
def check_https_enforcement():
    ...
    config_files = [f for f in glob.glob('**/*.py', recursive=True) 
                   if not any(exclude in f for exclude in ['node_modules', '.next', 'dist', 'build', '__pycache__', '.git'])]
