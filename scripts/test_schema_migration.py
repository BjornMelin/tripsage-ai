...
try:
    ...
    from tripsage_core.models.db.api_key import ApiKeyCreate, ApiKeyDB
    from tripsage_core.models.db.chat import ChatSessionDB
    from tripsage_core.models.db.memory import Memory, MemoryCreate, SessionMemory
...
def test_memory_models():
    ...
    try:
        ...
        assert isinstance(memory.user_id, UUID), (
            f"user_id should be UUID, got {type(memory.user_id)}"
        )
        ...
        assert isinstance(session_memory.user_id, UUID), (
            f"user_id should be UUID, got {type(session_memory.user_id)}"
        )
        ...
        assert isinstance(memory_create.user_id, UUID), (
            f"user_id should be UUID, got {type(memory_create.user_id)}"
        )
...
def test_api_key_models():
    ...
    try:
        ...
        assert isinstance(api_key.user_id, UUID), (
            f"user_id should be UUID, got {type(api_key.user_id)}"
        )
        ...
        assert isinstance(api_key_create.user_id, UUID), (
            f"user_id should be UUID, got {type(api_key_create.user_id)}"
        )
...
def test_chat_models():
    ...
    try:
        ...
        assert isinstance(chat_session.user_id, UUID), (
            f"user_id should be UUID, got {type(chat_session.user_id)}"
        )
...
def test_uuid_validation():
    ...
    try:
        ...
        test_cases = ["invalid_uuid", "123", "", "not-a-uuid-at-all"]
