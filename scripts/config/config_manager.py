...
from tripsage_core.config import get_settings, validate_configuration, Settings
...
class ConfigManager:
    ...
    def generate_template(self, output_file: str, include_secrets: bool = False) -> bool:
        ...
        logger.info(f"📄 Generating configuration template...")
    ...
    def export_config(self, output_file: str, format_type: str = "env") -> bool:
        ...
        try:
            ...
            if format_type.lower() == "env":
                ...
                config_dict = settings.model_dump(exclude={'secret_key', 'database_service_key', 'database_jwt_secret', 'openai_api_key'})
...
Examples:
  ...
  %(prog)s export config.json --format json     # Export config as JSON
    ...
    template_parser.add_argument('--include-secrets', action='store_true', 
    ...
    security_parser.add_argument('--format', choices=['json', 'human'], default='human',
                                help='Output format')
    
    # Check environment command
    check_env_parser = subparsers.add_parser('check-env', help='Check configuration for environment')
    check_env_parser.add_argument('environment', choices=['development', 'production', 'test'],
                                 help='Target environment')
    
    # Export command
    export_parser = subparsers.add_parser('export', help='Export configuration')
    export_parser.add_argument('output', help='Output file path')
    export_parser.add_argument('--format', choices=['env', 'json'], default='env',
                               help='Export format')
