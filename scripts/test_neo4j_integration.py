...
from src.db.neo4j.sync import (
    sync_destinations_to_knowledge_graph,
    sync_trips_to_knowledge_graph,
)
...
async def test_connection():
    ...
    try:
        ...
        print(
            f"  - Relationship Types: {', '.join(stats['relationship_types'].keys())}"
        )
        return True
    ...
    try:
        ...
        for i, route in enumerate(routes, 1):
            ...
            for j, conn in enumerate(connections, 1):
                ...
                print(
                    f"  Leg {j}: {conn.get('from')} -> {conn.get('to')} "
...
async def test_memory_mcp():
    ...
    try:
        ...
        if details:
            print("✅ Retrieved entity details:")
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Test Neo4j knowledge graph integration"
    )
    ...
    list_parser = subparsers.add_parser(
        "list", help="List destinations in knowledge graph"
    )
    list_parser.add_argument(
        "--limit", type=int, default=10, help="Maximum number of destinations to list"
    )
    ...
    route_parser.add_argument(
        "--max-stops", type=int, default=2, help="Maximum number of intermediate stops"
    )
    ...
    rec_parser.add_argument(
        "--budget", type=int, choices=range(1, 6), help="Budget level (1-5)"
    )
