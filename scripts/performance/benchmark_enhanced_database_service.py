...
class DatabaseServiceBenchmark:
    ...
    async def _benchmark_service(
        ...
        try:
            ...
            error_rate = (
                (failures / total_operations * 100) if total_operations > 0 else 0
            )
            ...
            logger.info(
                f"Completed {service_type} benchmark: {operations_per_second:.2f} ops/sec, "
    ...
    async def _benchmark_lifo_vs_fifo(self):
        ...
        if fifo_result.p95_latency_ms > 0:
            latency_improvement = (
                (fifo_result.p95_latency_ms - lifo_result.p95_latency_ms)
                / fifo_result.p95_latency_ms
                * 100
            )
            logger.info(
                f"LIFO vs FIFO P95 latency improvement: {latency_improvement:.2f}%"
            )
    ...
    async def _benchmark_regression_detection(self):
        ...
        try:
            ...
            if "regression_detection" in metrics:
                ...
                logger.info(
                    f"Regression detection tracked metrics: {regression_metrics.get('tracked_metrics', 0)}"
                )
    ...
    def _analyze_results(self) -> Dict:
        ...
        for test_name, results in by_test.items():
            enhanced_lifo = next(
                (r for r in results if r.service_type == "enhanced_lifo"), None
            )
            enhanced_fifo = next(
                (r for r in results if r.service_type == "enhanced_fifo"), None
            )
            ...
            if enhanced_lifo and original:
                throughput_improvement = (
                    (
                        enhanced_lifo.operations_per_second
                        - original.operations_per_second
                    )
                    / original.operations_per_second
                    * 100
                )
                latency_improvement = (
                    (original.p95_latency_ms - enhanced_lifo.p95_latency_ms)
                    / original.p95_latency_ms
                    * 100
                )
            ...
            if enhanced_lifo and enhanced_fifo:
                lifo_vs_fifo_latency = (
                    (enhanced_fifo.p95_latency_ms - enhanced_lifo.p95_latency_ms)
                    / enhanced_fifo.p95_latency_ms
                    * 100
                )
        ...
        if analysis["performance_comparison"]:
            ...
            for test, data in analysis["performance_comparison"].items():
                if "throughput_improvement_percent" in data:
                    print(
                        f"{test}: {data['throughput_improvement_percent']:.1f}% throughput improvement"
                    )
