...
async def example_basic_benchmark():
    ...
    if results.optimized_query_metrics:
        best_metric = min(results.optimized_query_metrics, key=lambda x: x.avg_latency_ms)
...
def example_ci_simulation():
    ...
    try:
        ...
        ci_command = [
            ...
            "full-pipeline",
            "--git-commit", "example-commit-123",
            "--quick",
            "--output-dir", "./ci_example_results"
        ]
...
if __name__ == "__main__":
    ...
    for directory in ["./example_results", "./custom_results", "./dev_results", "./ci_example_results"]:
