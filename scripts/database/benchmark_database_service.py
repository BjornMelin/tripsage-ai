...
class DatabaseBenchmark:
    ...
    async def _benchmark_connection(self, service: ConsolidatedDatabaseService, name: str):
    ...
    async def _benchmark_queries_by_mode(self, service: ConsolidatedDatabaseService, mode: ConnectionMode):
        ...
        for i in range(min(10, self.iterations)):
            ...
            try:
                await service.insert(
                    "test_benchmark",
                    {"name": f"Benchmark {i}", "created_at": datetime.utcnow().isoformat()},
                    mode=mode
                )
                duration = (time.time() - start) * 1000
                result.add_result(duration)
    ...
    async def _benchmark_concurrent_operations(self, service: ConsolidatedDatabaseService):
        """Benchmark concurrent operations."""
        await service.connect()
        
        result = BenchmarkResult(f"Consolidated - Concurrent SELECT ({self.concurrent_users} users)")
    ...
    async def _benchmark_vector_search(self, service: ConsolidatedDatabaseService):
    ...
    def generate_report(self):
        ...
        headers = ["Operation", "Avg (ms)", "Min (ms)", "Max (ms)", "P95 (ms)", "Success", "Count", "Notes"]
